...
process.on("uncaughtException", (err) => {
  ...
  logErrorToFile("UNCAUGHT_EXCEPTION", err?.stack || err?.message || err);
});
...
function logErrorToFile(type: string, message: any, ...args: any[]) {
  try {
    const timestamp = new Date().toISOString();
    let msgStr = typeof message === 'string' ? message : JSON.stringify(message, Object.getOwnPropertyNames(message));
    let argsStr = args.map(a => typeof a === 'string' ? a : JSON.stringify(a, Object.getOwnPropertyNames(a))).join(' ');
    const logLine = `[${timestamp}] [${type}] ${msgStr} ${argsStr}\n`;
    fs.appendFileSync(path.join(process.cwd(), 'aistudio-errors.log'), logLine);
  } catch (e) {
...
function getWebTorrentClient() {
  if (!_client) {
    ...
    try {
      ...
      console.error(
        "[WEBTOR] Critical: failed to initialize WebTorrent client:",
        e,
      );
    }
  }
...
const bootstrapTorrentReady = (torrent: any) => {
  ...
  const bootstrapTimer = setInterval(() => {
    ...
    console.log(
      `[WEBTOR-BOOTSTRAP] Forcing fast announce for ${torrent.name || torrent.infoHash} (attempt ${bootstrapCount})...`,
    );
    try {
      PUBLIC_TRACKERS.forEach((tr) => {
      ...
      if (
        torrent.discovery &&
        torrent.discovery.tracker &&
        typeof torrent.discovery.tracker.announce === "function"
      ) {
        torrent.discovery.tracker.announce();
      }
    } catch (e: any) {
      console.log(
        "[WEBTOR-BOOTSTRAP] Error in force announce:",
        e.message || e,
      );
    }
...
const addTorrentToClient = (task: TorrentTask): any => {
  ...
  const existing = client.torrents.find(
    (t: any) =>
      (magnetHash &&
        t.infoHash &&
        t.infoHash.toLowerCase() === magnetHash.toLowerCase()) ||
      t.magnetURI === magnet,
  );
  ...
  try {
    ...
    console.log(
      `[WEBTOR] Initiated torrent add: ${task.name} (${torrent.infoHash || "Pending Hash"})`,
    );
    ...
    const stuckLogger = setInterval(() => {
      if (!torrent.ready) {
        console.log(
          `[WEBTOR] Still waiting for metadata: ${task.name} - Peers: ${torrent.numPeers}`,
        );
        ...
        console.log(
          `[WEBTOR] Active but 0 speed: ${task.name} - Progress: ${(torrent.progress * 100).toFixed(1)}% - Peers: ${torrent.numPeers}`,
        );
      }
    }, 60000);
    ...
    torrent.on("metadata", () => {
      console.log(
        `[WEBTOR] Metadata received for: ${task.name} (${torrent.infoHash})`,
      );
    });
...
const restartTorrent = (task: TorrentTask): any => {
  ...
  const existing = client.torrents.find(
    (t: any) => t.magnetURI === task.magnetLink || t.infoHash === task.infoHash,
  );
  if (existing) {
    console.log(
      `Restarting stalled torrent: ${task.name} (Attempt ${t.retryCount}/${MAX_RETRY_ATTEMPTS})`,
    );
    try {
      client.remove(existing);
    } catch (e: any) {
      console.error(
        `Error removing existing stalled torrent client:`,
        e.message || e,
      );
    }
  }
...
app.post("/api/aistudio-logs", (req, res) => {
  try {
    const { type, message, args } = req.body;
    logErrorToFile(type || "CLIENT_ERROR", message, ...(args || []));
...
app.use((req, res, next) => {
  ...
  try {
    fs.appendFileSync(
      "proxy.log",
      "[EXPRESS URL LOG] " + req.method + " " + req.url + "\n",
    );
  } catch (e) {}
  next();
});
...
async function checkTrackerHealth(
  url: string,
): Promise<{ status: "online" | "offline"; error?: string }> {
  try {
    ...
    if (response.status === 403) {
      return {
        status: "online",
        error: "Cloudflare Protected (But reachable)",
      };
    }
...
async function runHealthChecks() {
  ...
  await Promise.all(
    enabledIndexers.map(async (indexer: any) => {
      try {
...
async function resumeActiveTorrents() {
  const db = loadDB();
  const downloading = db.torrentTasks.filter(
    (t: TorrentTask) =>
      (t.status === "downloading" ||
        t.status === "connecting" ||
        t.status === "stalled") &&
  ...
  for (const task of downloading) {
    if (task.magnetLink && task.magnetLink.startsWith("magnet:")) {
      const magnet = injectTrackers(task.magnetLink);
      const existing = client.torrents.find(
        (t: any) => t.magnetURI === magnet || t.infoHash === task.infoHash,
      );
      if (existing) {
...
async function fixStuckBooks() {
  ...
  for (const book of db.books) {
    ...
    if (
      ...
      firstChapter.content.includes("is being processed")
    ) {
      console.log(`Fixing stuck book: ${book.title}`);
      try {
        // Reduced timeout for startup fix to keep things moving
        const enriched = await enrichTorrentTaskWithMetadata(
          book.title,
          false,
          book.size,
          "Auto-Repair",
  ...
  if (changed) {
    saveDB(db);
    console.log(
      "Cleanup migration: Stuck book placeholders repaired successfully.",
    );
  }
}
...
setTimeout(async () => {
  runHealthChecks().catch((err) =>
    console.error("Error in startup runHealthChecks:", err),
  );
  fixStuckBooks().catch((err) =>
    console.error("Error in startup fixStuckBooks:", err),
  );
  resumeActiveTorrents().catch((err) =>
    console.error("Error in startup resumeActiveTorrents:", err),
  );
}, 5000);
...
function finalizeFileLocation(
  ...
  console.log(
    `[STORAGE] Keeping file in server staging area: ${sourcePath}. File will be organized client-side on device storage.`,
  );
  return sourcePath;
}
    ...
    if (fs.existsSync(DB_FILE)) {
      ...
      defaultIndexers.forEach((def) => {
        ...
        if (!existing && !wasDeleted) {
          ...
          (existing.url.includes(".lu") ||
            existing.url.includes(".cm") ||
            existing.url.includes(".so") ||
            existing.url.includes("kickasstorrents.to"))
        ) {
...
async function resolveLibGenDownload(mirrorUrl: string): Promise<string> {
  try {
    ...
    const getLink =
      $('a:contains("GET")').attr("href") || $("h2 a").attr("href");
    if (getLink) {
      return getLink.startsWith("http")
        ? getLink
        : new URL(getLink, mirrorUrl).toString();
    }
  } catch (err) {
...
async function extractEpubContent(filePath: string): Promise<any[]> {
  ...
  if (stats.size < 100) {
    console.error(
      `[EPUB-ERROR] File is suspiciously small (${stats.size} bytes)`,
    );
    return [];
  }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      epub.on("end", async () => {
        console.log(
          `[EPUB-DEBUG] EPUB parsed. Metadata:`,
          epub.metadata?.title,
        );
          ...
          try {
            ...
            if (rawContent && rawContent.trim()) {
              ...
              const tocItem = epub.toc?.find(
                (t: any) => t.id === section.id || t.href?.includes(section.id),
              );
            ...
            console.error(
              `[EPUB-ERROR] Failed to extract section ${i} (${section.id}):`,
              err.message || err,
            );
          }
        }
        ...
        console.log(
          `[EPUB-DEBUG] Extraction complete. Found ${chapters.length} chapters.`,
        );
        resolve(chapters);
      });
 ...
 */
async function enrichTorrentTaskWithMetadata(
  torrentTitle: string,
  isAudiobook: boolean,
  size: string,
  indexer: string,
  ...
  try {
    ...
    for (let task of db.torrentTasks) {
      ...
      if (
        ...
        const activeTorrent = client.torrents.find(
          (t: any) =>
        ...
        if (activeTorrent) {
          ...
          if (activeTorrent.downloadSpeed > 1024 * 1024) {
            downloadSpeed =
              (activeTorrent.downloadSpeed / (1024 * 1024)).toFixed(1) +
              " MB/s";
          } else {
            downloadSpeed =
              (activeTorrent.downloadSpeed / 1024).toFixed(1) + " KB/s";
          }
          ...
          files = task.files.map((f) => {
            const matchingFile = activeTorrent.files.find(
              (af: any) => af.name === f.name,
            );
            return {
              ...f,
              progress: matchingFile
                ? Math.round(matchingFile.progress * 100)
                : f.progress,
            };
          });
          ...
          if (activeTorrent.progress === 1) {
            ...
            const isStalled =
              (activeTorrent.downloadSpeed <= 0 &&
                activeTorrent.numPeers === 0 &&
                now - (task.zeroSpeedSince || now) > 90000) ||
              (activeTorrent.downloadSpeed <= 0 &&
                activeTorrent.numPeers > 0 &&
                now - (task.zeroSpeedSince || now) > 180000) ||
              (status === "connecting" &&
                now - (task.addedAt ? new Date(task.addedAt).getTime() : now) >
                  180000);
              ...
              if (task.status !== "stalled") {
                ...
                if (now - lastAnnounce > 150000) {
                  ...
                  try {
                    ...
                    console.log(
                      `[WEBTOR] Periodic re-announce for stalled torrent: ${task.name}`,
                    );
                  } catch (e) {}
                }
              }
          ...
          try {
            console.log(
              `Adding missing torrent from task manager: ${task.name}`,
            );
            addTorrentToClient(task);
          } catch (e: any) {
            console.error(
              `[WEBTOR] Safety catch: Failed to register missing torrent ${task.name}:`,
              e.message || e,
            );
          }
        }
        ...
        if (status === "completed" && task.status !== "completed") {
          const isAudio =
            task.name.toLowerCase().includes("audiobook") ||
            task.name.toLowerCase().includes("mp3") ||
            task.name.toLowerCase().includes("m4b");
            ...
            if (fs.existsSync(taskDir)) {
              if (fs.lstatSync(taskDir).isFile()) {
                ...
                if (!isAudio && actualFile.toLowerCase().endsWith(".epub")) {
                  console.log(
                    `[STREAMING] Automated extraction for ${task.name}...`,
                  );
                ...
                if (isAudio) {
                  ...
                  const matched = ebookExts.reduce(
                    (found: string | undefined, ext: string) => {
                      return (
                        found ||
                        items.find((f: string) => f.toLowerCase().endsWith(ext))
                      );
                    },
                    undefined,
                  );
          ...
          if (task.enrichedBook) {
            newBook = {
              ...
              chapters:
                realChapters.length > 0
                  ? realChapters
                  : fileMetadata.chapters || task.enrichedBook.chapters || [],
              duration:
                totalDuration ||
                task.enrichedBook.duration ||
            ...
            newBook = {
              ...
              description:
                ...
                `Downloaded from indexer ${task.indexer}.`,
              ...
              chapters:
                realChapters.length > 0
                  ? realChapters
                  : fileMetadata.chapters || [],
              filePath: foundPath || "",
              fileUrl: foundUrl || "",
              duration:
                totalDuration ||
                fileMetadata.duration ||
                (isAudio ? 18000 : undefined),
            };
          }
          ...
          if (!newBook.fileUrl && task.files && task.files.length > 0) {
            const ebookFile = task.files.find((f: any) =>
              f.name
                .toLowerCase()
                .match(/\.(epub|pdf|mobi|azw3|txt|djvu|mp3|m4b|aac)$/),
            );
            const nonImageFile =
              ebookFile ||
              task.files.find(
            ...
            newBook.filePath =
              newBook.filePath ||
              path.join(STORAGE_DIRS.download, task.name, nonImageFile.name);
          }
    ...
    console.error(
      "[SERVER] Critical error in downloading polling interval:",
      globalIntervalErr,
    );
  }
}, 5000);
...
app.get(["/api/proxy-hf/*", "/api/models/*"], async (req, res) => {
  fs.appendFileSync(
    "proxy.log",
    `[${new Date().toISOString()}] Incoming request: ${req.url}\n`,
  );
  ...
  if (targetPath.includes("onnx-community/vits-ljspeech")) {
    ...
    targetPath = targetPath.replace(
      "onnx-community/vits-ljspeech",
      "Xenova/vits-ljspeech",
    );
  }
  if (targetPath.includes("onnx-community/mms-tts-eng")) {
    console.log(`[PROXY] Rewriting legacy/incorrect model ID: ${targetPath}`);
    targetPath = targetPath.replace(
      "onnx-community/mms-tts-eng",
      "Xenova/mms-tts-eng",
    );
  }
  ...
  if (targetPath.includes("preprocessor_config.json")) {
    if (targetPath.toLowerCase().includes("kokoro")) {
      console.log(
        `[PROXY] Injecting safe dummy preprocessor_config for ${targetPath}`,
      );
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          `[PROXY] Global HF failed (${e.response?.status || "network error"}). Falling back to Mirror for: ${targetPath}`,
        );
        ...
        try {
          ...
          console.error(
            `[PROXY] Mirror failed (${(mirrorError.response?.status || "network error", mirrorError.message)}). Last resort: ModelScope.`,
          );
          ...
          try {
            ...
            console.warn(
              `[PROXY] ModelScope also failed (${modelScopeError.response?.status || "network error"}).`,
            );
            const finalStatus = modelScopeError.response?.status || 404;
            const finalError: any = new Error(
              `All proxies failed for ${targetPath}`,
            );
    ...
    if (!response)
      throw new Error("Proxy returned empty response after fallbacks");
    ...
    const mappedStatus =
      origStatus === 401 || origStatus === 403 ? 404 : origStatus;
    if (mappedStatus === 404) {
      return res
        .status(404)
        .send("Not Found on Hugging Face (Check your Model ID)");
    }
    console.error(
      `[PROXY] All attempts failed for ${targetPath}:`,
      err.message,
    );
...
app.post("/api/books/sync-all", async (req, res) => {
  const db = loadDB();
  const booksToSync = db.books.filter(
    (b: Book) =>
      !b.description ||
      b.description.includes("Manually cataloged") ||
      b.description.includes("No metadata found") ||
      !b.coverUrl ||
      b.coverUrl.includes("unsplash"),
  );
...
app.post("/api/scan-library", async (req, res) => {
  ...
  if (!fs.existsSync(indexerDir)) {
    try {
      ...
      return res
        .status(500)
        .send("Watch folder does not exist and could not be created.");
    }
  }
  ...
  const scanDir = async (dir: string, depth = 0) => {
    ...
    for (const item of items) {
      ...
      if (stats.isDirectory()) {
        ...
        if (lower.match(/\.(mp3|m4b|aac|flac|wav|ogg|m4a)$/)) {
          const alreadyExists = db.books.some((b: Book) => {
            if (
              b.filePath &&
              (b.filePath === fullPath ||
                b.filePath === item ||
                path.join(dir, b.filePath) === fullPath)
            )
              return true;
            if (
              b.fileUrl &&
              (b.fileUrl.endsWith(encodeURIComponent(item)) ||
                b.fileUrl.includes(encodeURIComponent(item)))
            )
              return true;
            return false;
          });
          if (!alreadyExists) audioFiles.push(item);
        } else if (lower.match(/\.(epub|pdf|mobi|azw3|txt|djvu)$/)) {
          const alreadyExists = db.books.some((b: Book) => {
            if (
              b.filePath &&
              (b.filePath === fullPath ||
          ...
          if (!alreadyExists) {
            console.log(`[SCANNER] Found new ebook item: ${item}`);
            const nameWithoutExt = item.replace(/\.[^/.]+$/, "");
            let epubChapters: any[] = [];
            let fileMeta: Partial<Book> = {
            ...
            const newBook: Book = {
              ...
              description:
                ...
                `Automatically imported from local folder: ${fullPath}`,
              ...
              chapters:
                epubChapters.length > 0
                  ? epubChapters
                  : fileMeta.chapters && fileMeta.chapters.length > 0
                    ? fileMeta.chapters
                    : undefined,
            };
            db.books.push(newBook);
    ...
    if (audioFiles.length > 0) {
      audioFiles.sort();
      console.log(
        `[SCANNER] Found ${audioFiles.length} new audio files in directory: ${dir}`,
      );
      const firstFile = audioFiles[0];
      const fullPath = path.join(dir, firstFile);
      const nameWithoutExt = firstFile.replace(/\.[^/.]+$/, "");

      const dirName = path.basename(dir);
      let authorGuess = "Unknown Author (Scanned)";
      let titleGuess = audioFiles.length > 1 ? dirName : nameWithoutExt;

      const dirParts = dir.split(path.sep);
      if (audioFiles.length > 1 && dirParts.length >= 2) {
        authorGuess = dirParts[dirParts.length - 2];
      }

      let fileMeta: Partial<Book> = { title: titleGuess, author: authorGuess };
      try {
        fileMeta = await getMetadataFromFile(fullPath);
      } catch (e) {}

      const mappedChapters = audioFiles.map((af, i) => {
        return {
          id: `chapter-${i}`,
          title: af.replace(/\.[^/.]+$/, ""),
          start: 0,
          end: 0,
          fileUrl: `/api/files/${encodeURIComponent(af)}`,
        };
      });

      const newBook: Book = {
        id: `book-scan-${crypto.randomUUID()}`,
        title: fileMeta.title || titleGuess,
        author: fileMeta.author || authorGuess,
        type: "audiobook",
        coverUrl:
          "https://images.unsplash.com/photo-1478737270239-2f02b77fc618?q=80&w=200&auto=format&fit=crop",
        description:
          fileMeta.description ||
          `Automatically imported audiobook from local folder: ${dir}`,
        genres: fileMeta.genres || ["Uncategorized"],
        progress: 0,
        currentTime: 0,
        currentPage: 0,
      ...
      if (fileName) {
            ...
            for (const item of items) {
              ...
              try {
                if (fs.lstatSync(fullPath).isDirectory()) {
                  ...
                  safeDecode(item) === safeDecode(fileName)
                ) {
                  return fullPath;
                }
              } catch (e) {
        ...
        for (const dir of searchDirs) {
          ...
          if (actualFile) {
            ...
            fileExists = true;
            db.books[
              db.books.findIndex((b: Book) => b.id === book.id)
            ].filePath = actualFile;
            saveDB(db);
            break;
          }
...
app.post("/api/books/:id/process", async (req, res) => {
  ...
  if (book.filePath.endsWith(".epub")) {
    ...
    if (chapters && chapters.length > 0) {
      ...
      return res
        .status(500)
        .json({ error: "EPUB extraction failed during re-process" });
    }
  }

  res
    .status(400)
    .json({ error: "File format not supported for deep processing" });
});
...
app.delete("/api/books/:id", (req, res) => {
  ...
  if (index !== -1) {
    ...
    if (deleteFiles && book.filePath && fs.existsSync(book.filePath)) {
      try {
        fs.unlinkSync(book.filePath);
        // Try to clean up empty parent directory if it's not a core library folder
        const parentDir = path.dirname(book.filePath);
        const filesInParent = fs.readdirSync(parentDir);
        if (filesInParent.length === 0) {
          fs.rmdirSync(parentDir); // clean up empty author/book folders
        }
      } catch (e) {
        console.error("Failed to delete file for", title, e);
      }
    }

    // Try deleting chapters if audiobook
    if (
      deleteFiles &&
      book.chapters &&
      book.type === "audiobook" &&
      ...
      try {
        const parentDir = path.dirname(book.filePath);
        // Since chapters might be alongside, we can check if it's a directory
        if (
          fs.existsSync(book.filePath) &&
          fs.lstatSync(book.filePath).isDirectory()
        ) {
          fs.rmSync(book.filePath, { recursive: true, force: true });
        }
      } catch (e) {}
    }
    ...
    db.logs.push({
      ...
      level: "warn",
      source: "server",
      message: `Removed track: Book [${title}]${deleteFiles ? " and local files deleted" : ""}.`,
    });
    saveDB(db);
...
app.get("/api/library/cleanup", (req, res) => {
  const db = loadDB();
  const missingEntries: Book[] = [];

  for (const book of db.books) {
    if (book.isDownloaded && book.filePath) {
      // Verify path exists
      let isValid = fs.existsSync(book.filePath);
      if (
        book.type === "audiobook" &&
        book.chapters &&
        book.chapters.length > 0
      ) {
        // For grouped audiobooks, the filePath may refer to a directory
        if (isValid) {
          const stat = fs.lstatSync(book.filePath);
          if (stat.isDirectory()) {
            // Dir exists. Are there audio files? We can assume it's valid if dir exists.
          }
        }
      }
      if (!isValid) {
  ...
  return res.json({ missingEntries });
});
...
app.get("/api/metadata/search", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `[Metadata Proxy Search Error] Failed to fetch OpenLibrary search for "${q}":`,
      err.message || err,
    );
    return res.status(500).json({
      error: "Failed to search metadata source",
      details: err.message,
    });
  }
});
...
app.get("/api/torrents/inspect", async (req, res) => {
  ...
  try {
    ...
    let infoTask =
      client.get(enhancedMagnet) || (infoHash ? client.get(infoHash) : null);
    ...
    const cleanup = () => {
      if (
        infoTask &&
        !loadDB().torrentTasks.find((t) => t.infoHash === infoTask.infoHash)
      ) {
        try {
          client.remove(infoTask.infoHash);
        } catch (e) {}
      }
    };
    ...
    const timeout = setTimeout(() => {
      res
        .status(408)
        .json({ error: "Metadata fetch timed out. Indexer might be slow." });
      cleanup();
    }, 90000);
...
app.post("/api/torrents", async (req, res) => {
  try {
    ...
    const exists = db.torrentTasks.find(
      (t: TorrentTask) => t.magnetLink === (magnetLink || downloadUrl),
    );
    if (exists) {
    ...
    const isAudio =
      type === "audiobook" ||
      safeTitle.toLowerCase().includes("audiobook") ||
      safeTitle.toLowerCase().includes("mp3") ||
      safeTitle.toLowerCase().includes("m4b");
      ...
      (async () => {
        try {
          ...
          if (success) {
            ...
            const task = innerDb.torrentTasks.find(
              (t: any) => t.id === newTask.id,
            );
            if (task) {
              task.progress = 100;
              ...
              const enrichedBook = await enrichTorrentTaskWithMetadata(
                safeTitle,
                isAudio,
                size,
                indexer,
    ...
    try {
      ...
      if (torrent) {
        const updateTaskWithMetadata = () => {
          const innerDb = loadDB();
          const t = innerDb.torrentTasks.find(
            (tt: any) => tt.id === newTask.id,
          );
          if (t) {
            t.infoHash = torrent.infoHash;
            t.name = torrent.name;
            t.files = torrent.files.map((f: any) => ({
              ...
              type:
                ...
                f.name.toLowerCase().endsWith(".m4b")
                  ...
                  : f.name
                        .toLowerCase()
                        .match(/\.(epub|pdf|mobi|azw3|txt|djvu)$/)
                    ? "ebook"
                    : f.name.toLowerCase().match(/\.(jpg|jpeg|png|webp|gif)$/)
                      ? "cover"
                      : "other",
            }));
            saveDB(innerDb);
    ...
    (async () => {
      try {
        const enrichedBook = await enrichTorrentTaskWithMetadata(
          ...
          isAudio,
          size || "Calculating...",
          indexer || "Bookrr Indexer",
        );
        const enrichDb = loadDB();
        const enrichTask = enrichDb.torrentTasks.find(
          (t: any) => t.id === newTask.id,
        );
        if (enrichTask) {
...
app.delete("/api/torrents/:id", (req, res) => {
  const db = loadDB();
  const index = db.torrentTasks.findIndex(
    (t: TorrentTask) => t.id === req.params.id,
  );
  if (index !== -1) {
    ...
    const torrent = client.torrents.find(
      (t: any) =>
        t.magnetURI === task.magnetLink || t.infoHash === task.infoHash,
    );
    if (torrent) {
  ...
  try {
    if (book.filePath.toLowerCase().endsWith(".epub")) {
      ...
      if (chapters.length > 0) {
        ...
        return res.json({
          success: true,
          message: `Extracted ${chapters.length} chapters`,
        });
      } else {
        return res
          .status(500)
          .json({ error: "Extraction resulted in zero chapters" });
      }
    } else {
...
app.get("/api/tts", async (req, res) => {
  ...
  try {
    console.log(
      `[TTS-API] Fetching speech from Google for length: ${text.length}`,
    );
  ...
  for (const dir of searchDirs) {
    ...
    if (fs.existsSync(rootPath) && fs.lstatSync(rootPath).isFile()) {
      ...
      if (isAudio) contentType = "audio/mpeg";
        ...
        contentType = `image/${lowerFileName.split(".").pop()?.replace("jpg", "jpeg")}`;
      ...
      return res.sendFile(rootPath, (err) => {
        if (err) {
          if (
            ...
            console.log(
              `[FILE-API] File transfer interrupted/aborted: ${rootPath}`,
            );
          } else {
      ...
      res.sendFile(foundPath, (err) => {
        if (err) {
          if (
            ...
            console.log(
              `[FILE-API] File transfer interrupted/aborted: ${foundPath}`,
            );
          } else {
...
app.post("/api/books/:id/organize", (req, res) => {
  ...
  const finalPath = finalizeFileLocation(
    book.filePath,
    book.type,
    book.author,
    book.title,
...
app.get(["/pwabuilder-sw.js", "/sw.js", "/service-worker.js"], (req, res) => {
  ...
  const filePath =
    possiblePaths.find((p) => fs.existsSync(p)) || possiblePaths[1];
...
app.get(
  ["/icon.svg", "/icon-maskable.svg", "/icon-monochrome.svg"],
  (req, res) => {
...
app.use("/api", (err: any, req: any, res: any, next: any) => {
  ...
  res.status(500).json({ error: err.message || "Internal Server Error", stack: process.env.NODE_ENV !== "production" ? err.stack : undefined });
});
