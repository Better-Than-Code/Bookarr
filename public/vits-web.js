const u = "https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0", B = "https://cdnjs.cloudflare.com/ajax/libs/onnxruntime-web/1.18.0/", x = "https://cdn.jsdelivr.net/npm/@diffusionstudio/piper-wasm@1.0.0/build/piper_phonemize", c = {
...
async function p(e, m) {
  if (e.match("https://huggingface.co") || e.match("/api/models") || e.indexOf("/models/") !== -1) {
    try {
      ...
      await cache.put(url, new Response(m));
      console.log("[Cache] Saved to Cache API:", url);
    } catch (n) {
      console.error("[Cache] Failed to save Cache:", n);
      try {
        ...
        console.error("[Cache] Failed OPFS fallback:", n2);
      }
    }
  }
}
  ...
  if (e.match("https://huggingface.co") || e.match("/api/models") || e.indexOf("/models/") !== -1) {
    try {
      ...
      if (res) {
          ...
          if (blob.size > 0) {
            const txt = await blob.slice(0, 20).text();
            if (txt.toLowerCase().includes("<!doctype html") || txt.toLowerCase().includes("<html")) {
                console.log("[Cache] Deleted corrupt HTML cache in Cache API");
                await cache.delete(url);
                throw new Error("Corrupt Cache");
            }
          }
          console.log("[Cache] Hit from Cache API:", url);
          return blob;
      }
    } catch {}
    try {
      ...
      const b = await (await n.getFileHandle(o)).getFile();
      if (b.size > 0) {
        const txt = await b.slice(0, 20).text();
        if (txt.toLowerCase().includes("<!doctype html") || txt.toLowerCase().includes("<html")) {
            console.log("[Cache] Deleted corrupt HTML cache in OPFS");
            await n.removeEntry(o);
            throw new Error("Corrupt Cache");
        }
      }
      console.log("[Cache] Hit from OPFS:", e);
      return b;
    } catch {
      return;
    }
...
async function S(e, m) {
  ...
  if (!n || !n.ok) throw new Error("HTTP error " + (n?.status || "network") + " fetching " + e);
...
async function N(e, m) {
  h = h ?? await import("./piper-DeOu3H9E.js"), _ = _ ?? await import("https://cdn.jsdelivr.net/npm/onnxruntime-web@1.18.0/+esm");
  ...
  }), r = e.speakerId || 0, s = i.audio.sample_rate, d = i.inference.noise_scale, g = i.inference.length_scale, U = i.inference.noise_w;
  if (!globalThis._piperSessionCache) globalThis._piperSessionCache = {};
  let y = globalThis._piperSessionCache[n];
  if (!y) {
    let k = await f(`${u}/${n}`, m);
    let buf = await k.arrayBuffer();
    const threads = Math.min(4, Math.max(1, (navigator.hardwareConcurrency || 4) - 1));
    _.env.wasm.numThreads = threads;
    _.env.wasm.simd = true;
    try {
      y = await _.InferenceSession.create(buf.slice(0), { 
        executionProviders: globalThis.piperBackend === 'wasm' ? ['wasm'] : ['webgpu', 'wasm'], 
        graphOptimizationLevel: 'basic',
        executionMode: 'sequential',
        intraOpNumThreads: threads
      });
    } catch(err) {
      console.warn("Optimized WebGPU/WASM failed, using default WASM...", err);
      y = await _.InferenceSession.create(buf, { executionProviders: ['wasm'] });
    }
    globalThis._piperSessionCache[n] = y;
  }
  const w = {
...
async function L() {
  ...
  try {
      ...
      for (const req of reqs) {
          const a = req.url.split("/").at(-1).split(".")[0];
          req.url.endsWith(".onnx") && a in c && n.push(a);
      }
  } catch {}
  if (n.length === 0) {
    try {
