...
export async function saveOfflineFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      reject(
        new Error("Failed to save file to local browser offline storage."),
      );
    };
  });
}

export async function updateOfflineFilePath(
  bookId: string,
  filePath: string,
): Promise<void> {
  const db = await getDB();
...
export async function getOfflineFile(
  ...
  if (handle) {
    try {
      ...
      console.warn(
        "Failed to read from filesystem handle, falling back to IndexedDB",
        e,
      );
    }
  }
...
export async function getOfflineBooksMap(): Promise<{
            ...
            if (hCursor) {
              if (!result[hCursor.key as string]) {
                result[hCursor.key as string] = {
                  ...
                  filePath:
                    hCursor.value.filePath ||
                    `Local Desktop Mapped / ${(hCursor.value.handle as FileSystemFileHandle).name}`,
                };
              }
              hCursor.continue();
            } else {
...
export async function saveDirectoryHandle(
  id: "watch" | "watch_internal" | "ebooks" | "audiobooks",
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(new Error(`Failed to save directory handle for ${id}`));
  });
}
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve(
          request.result
            ? (request.result.handle as FileSystemDirectoryHandle)
            : null,
        );
      };
      request.onerror = () =>
        reject(new Error(`Failed to read directory handle for ${id}`));
    });
  } catch (e) {
...
export async function deleteDirectoryHandle(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => resolve();
    request.onerror = () =>
      reject(new Error(`Failed to delete directory handle for ${id}`));
  });
}
...
export async function saveFileHandle(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => resolve();
    request.onerror = () =>
      reject(new Error(`Failed to save file handle for ${bookId}`));
  });
}
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve(
          request.result
            ? (request.result.handle as FileSystemFileHandle)
            : null,
        );
      };
      request.onerror = () =>
        reject(new Error(`Failed to read file handle for ${bookId}`));
    });
  } catch (e) {
