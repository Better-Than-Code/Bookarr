...
async function fetchWithRetry(
  ...
  if (Date.now() < cooldownUntil) {
    throw new Error(
      `${providerName} is cooling down due to previous rate limits`,
    );
  }
  ...
  try {
    ...
    if (status === 429) {
      console.error(
        `[Metadata] ${providerName} hit 429 rate limit. Setting 1-minute cooldown.`,
      );
    ...
    if (isTimeout && retries > 0) {
      ...
      console.warn(
        `[Metadata] ${providerName} timed out. Retrying in ${Math.round(waitTime)}ms...`,
      );
    ...
    console.error(
      `[Metadata] ${providerName} fetch failed: ${status || err.message}`,
    );
...
export async function enrichMetadata(
  ...
  let cleanTitle = result.title
    .replace(
      /\b(ebook|pdf|epub|mobi|azw3|audiobook|mp3|m4b|flac|aac|unabridged|retail|web|h128|h64|x264|1080p|720p|vppv|hq|brrip|dvd|mp4|aac2|ac3|multi|v\d+)\b/gi,
      "",
    )
    .replace(
      /\b(edition|revised|expanded|collection|complete|set|read by|readaloud|narrated by)\b/gi,
      "",
    )
    .replace(/[\[\(\{\]\)\}]/g, " ")
    .replace(/[^a-zA-Z0-9' -]/g, " ")
  ...
  const queryVariants = Array.from(new Set([cleanTitle, result.title])).filter(
    (q) => q.length > 2,
  );
      ...
      const providerPromises = providers.map(async (provider, index) => {
        ...
        try {
          ...
          for (const res of results) {
            ...
            const authorMatch = query
              .toLowerCase()
              .includes(res.author?.split(",")[0].toLowerCase().trim())
              ? 0.2
              : 0;
            ...
            candidates.push({
              ...
              score: titleScore + authorMatch + typeScore,
          ...
          const isCoolingDown =
            err.message.includes("cooling down") ||
            err.message.includes("rate limit");
          if (isCoolingDown || err.message.includes("timeout")) {
            console.warn(
              `[Metadata] Provider ${provider.name} currently limited: ${err.message}`,
            );
          } else {
            console.warn(
              `[Metadata] Provider ${provider.name} failed: ${err.message}`,
            );
          }
        }
      });
  ...
  if (candidates.length > 0) {
      ...
      try {
        ...
        if (data[bookKey]) {
          ...
          best.description =
            details.notes ||
            details.summary ||
            (Array.isArray(details.excerpts)
              ? details.excerpts[0]?.text
              : undefined) ||
            best.description;
    ...
    return {
      ...best,
      description:
        best.description?.length > 1500
          ? best.description.substring(0, 1500) + "..."
          : best.description,
      coverUrl:
        best.coverUrl ||
