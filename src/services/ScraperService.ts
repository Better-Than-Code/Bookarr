...
async function getMagnetLink(
  ...
  try {
    const detailsUrl = link.startsWith("http")
      ? link
    ...
    if (response.status === 403 || response.data.includes("Just a moment")) {
      console.warn(
        `${indexerName} details page blocked by Cloudflare (403) or checking (Just a moment)`,
      );
      return "";
    }
...
export async function searchNative(
  ...
  if (indexer.name === "1337x") {
    ...
    for (const mirror of mirrors) {
      ...
      try {
        ...
        if (
          response.status === 404 ||
          (response.status === 200 && !response.data.includes("table-list"))
        ) {
        ...
        const promises = rows.slice(0, 10).map(async (element) => {
          ...
          const size =
            $(element).find("td.coll-4").text().split(" ")[0] +
            " " +
            $(element)
              .find("td.coll-4")
              .find("span")
        ...
        const filtered = resolvedResults.filter(
          (r): r is TorrentSearchResult => r !== null,
        );
        ...
        if (err.code === "ERR_FR_TOO_MANY_REDIRECTS") {
          ...
          console.error(
            `Error scraping 1337x mirror ${cleanMirror}:`,
            err.message,
          );
        }
      }
    ...
    try {
      ...
      const promises = rows
        ...
        .map(async (element) => {
          ...
          if (title && link) {
            const magnetLink = await getMagnetLink(
              link,
              indexer.url,
              "LimeTorrents",
            );
  ...
  if (indexer.name === "TorrentDownloads") {
    try {
      ...
      const startIdx =
        rows.findIndex((row) => $(row).find("th").length > 0) + 1;
      ...
      const promises = rows
        ...
        .map(async (element) => {
          ...
          const seeds =
            parseInt($(element).find("td:nth-child(4)").text()) || 0;
          const peers =
            parseInt($(element).find("td:nth-child(5)").text()) || 0;
    ...
    for (const mirror of mirrors) {
      ...
      try {
        ...
        const promises = rows.slice(1, 11).map(async (element) => {
          ...
          const peersIdx = 6;
          const seeds =
            parseInt($(element).find("td").eq(seedsIdx).text()) || 0;
          const peers =
            parseInt($(element).find("td").eq(peersIdx).text()) || 0;

          if (title && (link || magnetLink)) {
            const finalMagnet =
              magnetLink ||
              (link
                ? await getMagnetLink(link, cleanMirror, "GloTorrents")
                : "");
            return {
        ...
        if (
          ...
          console.error(
            `Error scraping GloTorrents mirror ${cleanMirror}:`,
            err.message,
          );
        }
      }
  ...
  if (indexer.name === "Kickass") {
    try {
      const categoryAppender =
        type === "ebook" ? " category:books" : " category:audiobooks";
      ...
      const promises = rows.slice(0, 10).map(async (element) => {
        ...
        const magnetLink = $(element)
          .find('a[title="Torrent magnet link"]')
          .attr("href");
        ...
        if (title && (link || magnetLink)) {
          const finalMagnet =
            magnetLink ||
            (link ? await getMagnetLink(link, indexer.url, "Kickass") : "");
          return {
    ...
    for (const mirror of mirrors) {
      ...
      try {
        ...
        if (
          ...
          console.error(
            `Error scraping LibGen mirror ${cleanMirror}:`,
            err.message,
          );
        }
      }
