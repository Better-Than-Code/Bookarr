  ...
  for await (const entry of (dirHandle as any).values()) {
    ...
    if (entry.kind === 'file') {
      ...
      if (entryPath.split('/').length <= 3) {
        await scanWatchFolder(subDirHandle, entryPath, fileList, currentRootHandle);
      }
    }
  }
  return fileList;
...
export function autoMapFiles(files: WatchFolderFile[], books: Book[]): WatchFolderFile[] {
  return files.map(file => {
    ...
    const cleanFileName = (file.type === 'cover' ? file.path : file.name).toLowerCase().replace(/[^a-z0-9]/g, ' ');
...
async function cleanupEmptyFolders(rootDir: FileSystemDirectoryHandle, pathParts: string[]): Promise<void> {
  try {
 ...
 */
export async function readFileBufferSafely(handle: FileSystemFileHandle, retries = 3): Promise<{ fileObj: File, buffer: ArrayBuffer }> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const isStaleErr = err.name === 'InvalidStateError' || 
      ...
      if (isStaleErr && attempt < retries) {
        console.warn(`[STORAGE] File snapshot stale for "${handle.name}" on attempt ${attempt}/${retries}. Retrying in ${attempt * 250}ms...`);
  ...
  throw new Error(`Unable to read file "${handle.name}" due to persistent state changes on disk.`);
}
...
export async function reorganizeFile(
    ...
    try {
        ...
        if (writtenFile.size !== file.size) {
            throw new Error(`Integrity size check failed: expected ${file.size} bytes, wrote ${writtenFile.size} bytes.`);
        }
...
export async function organizeSingleFile(
  ...
  try {
    ...
    const finalFileName = file.type === 'cover' 
    ...
    if (file.originDirHandle === destDir && file.path === internalExpectedRelative) {
        return {
          success: true,
...
export async function autoRelinkLibrary(
  ...
  saveFileHandle: (bookId: string, handle: FileSystemFileHandle, filePath?: string) => Promise<void>,
