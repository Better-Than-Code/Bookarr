...
export default function EbookReader({
  ...
  const [localFileContent, setLocalFileContent] = useState<ArrayBuffer | null>(
    null,
  );
  ...
  const isUnsupportedEbook =
    displayUrl.toLowerCase().endsWith(".mobi") ||
    displayUrl.toLowerCase().endsWith(".azw3") ||
    displayUrl.toLowerCase().endsWith(".djvu");
  const isEpub =
    displayUrl.toLowerCase().endsWith(".epub") ||
    (book.type === "ebook" && !isPdf && !isTxt && !isUnsupportedEbook);
  ...
  const isNativeView =
    (hasRealFile && chapters.length === 0 && !isEpub && !isUnsupportedEbook) ||
    isPdf;
  ...
  const findTocItemByHref = (tocList: any[], href: string): any => {
    ...
    for (const item of tocList) {
      if (item.href) {
        ...
        if (
          itemPath === targetPath ||
          targetPath.endsWith(itemPath) ||
          itemPath.endsWith(targetPath)
        ) {
          return item;
        }
      }
      ...
      if (currentLocation && currentLocation.start) {
        ...
        if (toc && toc.length > 0) {
          ...
          if (item) {
            ...
            const tIdx = toc.findIndex(
              (x) => x.id === item.id || x.href === item.href,
            );
            if (tIdx !== -1) {
    ...
    (textToFind: string) => {
      ...
      if (isEpub) {
        ...
        if (scrollContainerRef.current) {
          targetElement = findElementContainingText(
      ...
      try {
        ...
        const isVisible = (() => {
          try {
            ...
            return (
              rect.top >= viewHeight * 0.15 && rect.bottom <= viewHeight * 0.85
            );
          } catch (e) {
            return false;
          }
  ...
  useEffect(() => {
    if (readAloudState.isSpeaking && readAloudState.bookId === book.id) {
      ...
      if (readAloudState.sentences[readAloudState.currentIndex]) {
        highlightAndScrollToText(
          readAloudState.sentences[readAloudState.currentIndex],
        );
      }
    }
    ...
    readAloudState.isSpeaking,
    readAloudState.bookId,
    book.id,
    activeChapterIndex,
    chapters.length,
    registerCallback,
  ...
  const [activeTab, setActiveTab] = useState<
    "contents" | "bookmarks" | "highlights" | "info"
  >("contents");
  const [showSettings, setShowSettings] = useState(false);
  const [settingsTab, setSettingsTab] = useState<"font" | "layout" | "themes">(
    "font",
  );
  ...
  const isTtsSpeaking =
    readAloudState.isSpeaking && readAloudState.bookId === book.id;
  ...
  const [readingStats, setReadingStats] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : { timeSpent: 0, wordsRead: 0, lastSession: new Date().toISOString() };
  });
  ...
  const [activeSelectionText, setActiveSelectionText] = useState<string | null>(
    null,
  );
  const [activeSelectionCfi, setActiveSelectionCfi] = useState<string | null>(
    null,
  );
  const [annotationNote, setAnnotationNote] = useState<string>("");
  const [selectedHighlightColor, setSelectedHighlightColor] = useState<
    "amber" | "emerald" | "rose" | "sky" | "underline"
  >("amber");
      ...
      if (hl.cfiRange) {
        ...
        try {
          rendition.annotations.add(
            ...
            { id: hl.id },
            null,
            "epub-selected-annotation",
            {
              fill: fillStyle,
              outline: borderStyle,
              "border-bottom":
                hl.color === "underline" ? "2px dashed #f59e0b" : "none",
            },
          );
        } catch (e) {
  ...
  const handleSaveHighlight = () => {
    ...
    if (isEpub && activeSelectionCfi && renditionRef.current) {
      try {
        ...
        if (selectedHighlightColor === "emerald")
          fillStyle = "rgba(52, 211, 153, 0.4)";
        else if (selectedHighlightColor === "rose")
          fillStyle = "rgba(248, 113, 113, 0.4)";
        else if (selectedHighlightColor === "sky")
          fillStyle = "rgba(96, 165, 250, 0.4)";
        ...
        renditionRef.current.annotations.add(
          ...
          "epub-selected-annotation",
          {
            ...
            outline: borderStyle,
            "border-bottom":
              selectedHighlightColor === "underline"
                ? "2px dashed #f59e0b"
                : "none",
          },
        );
  ...
  const currentChapter = chapters[activeChapterIndex] || {
    title: "No Content",
    content: "This book has no chapters configured.",
  };
  ...
  const computedProgress = isEpub
    ? epubProgress
    : chapters.length > 0
      ? Math.floor(((activeChapterIndex + 1) / chapters.length) * 100)
      : 0;
    ...
    if (isEpub) {
      ...
      if (activeChapterIndex < chapters.length - 1) {
        ...
        onUpdateProgress(
          book.id,
          next,
          Math.floor(((next + 1) / chapters.length) * 100),
        );
      }
    }
  };
    ...
    if (isEpub) {
      ...
      if (activeChapterIndex > 0) {
        ...
        onUpdateProgress(
          book.id,
          prev,
          Math.floor(((prev + 1) / chapters.length) * 100),
        );
      }
    }
  };
    ...
    if (existing) {
      ...
      updated = [
        ...bookmarks,
        {
          ...
          label: isEpub
            ? currentChapterTitle
            : currentChapter.title || "Untitled Segment",
          createdAt: new Date().toISOString(),
        },
      ];
    }
    ...
    chapters.forEach((ch, chIdx) => {
      ...
      while (index !== -1) {
        results.push({
          ...
          excerpt: ch.content.substring(
            Math.max(0, index - 20),
            Math.min(ch.content.length, index + query.length + 30),
          ),
          index,
        });
        index = ch.content
          .toLowerCase()
          .indexOf(query.toLowerCase(), index + 1);
  ...
  const renderTocList = (items: any[], depth = 0): React.ReactNode => {
    return items.map((ch, idx) => {
      const isSelected =
        currentChapterTitle === ch.label ||
        (renditionRef.current &&
          renditionRef.current.currentLocation()?.start?.href?.split("#")[0] ===
            ch.href?.split("#")[0]);
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 truncate">
              {depth === 0 && (
                <span
                  className={`w-5 h-5 shrink-0 flex items-center justify-center text-[9px] font-mono rounded-lg border ${isSelected ? "border-amber-500/50 bg-amber-500/10" : "border-neutral-500/20 opacity-50"}`}
                >
                  {idx + 1}
                </span>
              )}
              <span
                className={`text-xs tracking-tight truncate ${isSelected ? "font-bold font-serif" : "font-medium opacity-80"}`}
              >
                {ch.label}
              </span>
            </div>
          ...
          {ch.subitems &&
            ...
            renderTocList(ch.subitems, depth + 1)}
        </div>
      );
    });
  };
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex flex-col overflow-hidden ${activeStyle.bg} ${activeStyle.text} transition-colors duration-500 font-sans`}
    >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col truncate max-w-[150px] sm:max-w-[300px]">
                ...
                <p className={`text-[10px] ${activeStyle.sub} italic truncate`}>
                  {isEpub ? currentChapterTitle : currentChapter.title}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className={`p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-full transition-colors ${isBookmarked ? "text-amber-500" : ""}`}
              >
                <Bookmark
                  size={18}
              ...
              >
                <Volume2 size={18} />
                {readAloudState.isSpeaking &&
                  readAloudState.bookId === book.id &&
                  !readAloudState.isPaused && (
                    <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  )}
              </button>
              <button
                onClick={() => setShowSettings(!showSettings)}
                className={`p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-full transition-colors ${showSettings ? "bg-black/5 dark:bg-white/5" : ""}`}
              >
                <Settings size={18} />
              </button>
              <button
      ...
      >
        {!isOfflineChecked ? (
          ...
          <div className="flex items-center justify-center w-full h-full opacity-50 uppercase tracking-widest font-bold text-xs">
            No book file found.
          </div>
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              ...
              >
                ...
                <div className="space-y-4">
                  <h2 className="text-2xl font-bold italic font-serif">
                    Format Not Supported
                  </h2>
                  <p className={`text-sm ${activeStyle.sub} leading-relaxed`}>
                    The web reader currently cannot render this file type
                    directly in the browser. Please download the file to read it
                    in your preferred e-reader application.
                  </p>
                </div>
          ...
          <div
            className={`w-full h-full transition-all duration-500 ${immersiveMode ? "pt-2 pb-2" : "pt-14 pb-16"} relative ${activeStyle.bg}`}
            style={{ height: "100vh" }}
          >
            <div className="w-full h-full relative">
              <ReactReader
                ...
                loadingView={
                  <div
                    className={`flex items-center justify-center h-full w-full ${activeStyle.bg} ${activeStyle.text}`}
                  >
          ...
          <div
            className={`mx-auto w-full h-full flex flex-col pt-20 pb-32 ${marginStyles[margins]} transition-all duration-500`}
          >
            {extractionFailed ? (
              <div className="flex-1 flex items-center justify-center p-6">
                <motion.div
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className={`text-sm ${activeStyle.sub} leading-relaxed`}>
                      We were unable to extract the text content from this EPUB
                      file. The file remains in your local storage, but the
                      reader system cannot render it in interactive mode.
                    </p>
                  </div>
                  <div className="flex flex-col gap-3">
                    <button
                  ...
                  {isPdf && (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-2 py-1 rounded bg-white hover:bg-neutral-100 border border-neutral-200"
                      >
                        A-
                      </button>
                      ...
                      <button
                        onClick={() =>
                          setPdfScale((s) => Math.min(3.0, s + 0.25))
                        }
                        className="px-2 py-1 rounded bg-white hover:bg-neutral-100 border border-neutral-200"
                      >
                        A+
                  ...
                  <div className="flex-1 overflow-auto w-full custom-scrollbar relative flex justify-center p-4">
                    ...
                    >
                      {Array.from(new Array(numPdfPages || 0), (el, index) => (
                        <div
                          key={`page_${index + 1}`}
                          className="mb-8 shadow-xl bg-white flex justify-center border border-neutral-300"
                        >
                          <Page
                            pageNumber={index + 1}
                            scale={pdfScale}
              ...
              <motion.article
                ...
                style={{
                  ...
                  columnGap: "4rem",
                  columnRule: isDualPage
                    ? `1px solid ${activeStyle.border.replace("border-", "")}`
                    : "none",
                }}
              >
                <div
                  className={`text-center mb-16 space-y-4 py-8 border-b-2 ${activeStyle.border}`}
                >
                  <p
                    className={`text-xs tracking-[0.2em] font-medium uppercase ${activeStyle.sub}`}
                  >
                    Chapter {activeChapterIndex + 1}
                  </p>
                  <h2 className="text-3xl sm:text-4xl font-bold italic tracking-tight font-serif">
                ...
                <div className="flex flex-col items-center gap-10 py-20">
                  ...
                  <div
                    className={`text-[11px] ${activeStyle.sub} italic flex flex-col items-center gap-1`}
                  >
                    <span>
          ...
          >
            ...
            <div className="relative group w-full px-4 -mt-2">
              <AnimatePresence>
                {scrubPreview !== null && (
                  ...
                  >
                    <p
                      className={`text-[9px] font-bold ${activeStyle.sub} uppercase tracking-widest mb-1`}
                    >
                      JUMP TO Chapter {scrubPreview + 1}
                    </p>
                    <p className="text-xs font-bold truncate max-w-[180px]">
                      {chapters[scrubPreview]?.title}
                    </p>
                    <div
                      className={`absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1 w-2 h-2 rotate-45 ${activeStyle.panel} border-r border-b ${activeStyle.border}`}
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  onUpdateProgress(
                    book.id,
                    idx,
                    Math.floor(((idx + 1) / (chapters.length || 1)) * 100),
                  );
                }}
                onClick={(e) => e.stopPropagation()}
            ...
            <div className="flex-1 flex items-center justify-between px-4 sm:px-10 text-[11px] font-medium tracking-tight">
              <div className="flex items-center gap-6">
                <div className="flex flex-col">
                  ...
                  <span className="font-bold">
                    {activeChapterIndex + 1} / {totalChapters} Ch
                  </span>
                </div>
                <div className="hidden sm:flex flex-col">
                  <span className={activeStyle.sub}>EST. TIME LEFT</span>
                  <span className="font-bold">
                    ~{Math.ceil((totalChapters - activeChapterIndex) * 15)} min
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col items-end">
                ...
                <span
                  className={`font-bold flex items-center gap-1.5 ${activeStyle.accentText}`}
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${theme === "amoled" ? "bg-neutral-500" : "bg-amber-500"} animate-pulse`}
                  />
                  READING
                </span>
              </div>
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-neutral-500/10 flex justify-between items-start">
                ...
                <button
                  onClick={() => setShowMenu(false)}
                  className={`p-1.5 rounded-lg ${activeStyle.border} border opacity-50 hover:opacity-100`}
                >
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="flex p-2 gap-1 border-b border-neutral-500/10 text-neutral-400">
                {(["contents", "bookmarks", "highlights", "info"] as const).map(
                  (tab) => (
                    <button
                      key={tab}
              ...
              <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                {activeTab === "contents" && (
                  <div className="space-y-1">
                    {isEpub ? (
                      toc.length === 0 ? (
                        <div className="py-20 text-center opacity-30 text-xs italic">
                          Loading chapters from EPUB...
                        </div>
                      ) : (
                        renderTocList(toc)
                      )
                      ...
                      chapters.map((ch, idx) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              onUpdateProgress(
                                book.id,
                                idx,
                                Math.floor(((idx + 1) / chapters.length) * 100),
                              );
                            }}
                          ...
                          >
                            <div className="flex items-center gap-3 truncate">
                              <span
                                className={`w-6 h-6 shrink-0 flex items-center justify-center text-[10px] font-mono rounded-lg border ${activeChapterIndex === idx ? "border-amber-500/50 bg-amber-500/10" : "border-neutral-500/20 opacity-50"}`}
                              >
                                {idx + 1}
                              </span>
                              <span
                                className={`text-sm tracking-tight truncate ${activeChapterIndex === idx ? "font-bold" : "font-medium opacity-80"}`}
                              >
                                {ch.title}
                              </span>
                            </div>
                            <div
                              className={`text-[10px] font-mono shrink-0 opacity-40 flex items-center gap-1`}
                            >
                              <Clock size={10} />
                              {readingTime}m
                            </div>
                  ...
                  <div className="space-y-3">
                    {highlights.length === 0 ? (
                      ...
                      highlights.map((hl) => (
                        <div
                          key={hl.id}
                          className={`p-4 rounded-xl border ${activeStyle.border} bg-black/5 space-y-2`}
                        >
                          <div className="flex justify-between items-start">
                            <div className="flex gap-1">
                              <div
                                className={`w-1 h-4 rounded-full ${hl.color === "amber" ? "bg-amber-500" : hl.color === "emerald" ? "bg-emerald-500" : hl.color === "rose" ? "bg-rose-500" : hl.color === "sky" ? "bg-sky-500" : "bg-neutral-500"}`}
                              />
                              <span
                                className={`text-[9px] font-bold uppercase ${activeStyle.sub}`}
                              >
                              ...
                              onClick={() => {
                                if (
                                  ...
                                  try {
                                    ...
                                    console.error(
                                      "Failed to remove live annotation style:",
                                      e,
                                    );
                                  }
                                }
                          ...
                          {hl.note && (
                            <div className="p-2.5 rounded bg-amber-500/5 border border-amber-500/10 text-[11px] text-neutral-300 leading-normal flex flex-col gap-1">
                              <span className="font-bold uppercase tracking-wider text-[8px] text-amber-500">
                                Annotation Note
                              </span>
                  ...
                  <div className="space-y-3">
                    {bookmarks.length === 0 ? (
                      <div className="py-10 text-center opacity-30 text-xs italic">
                        No items stored in library bookmarks.
                      </div>
                    ) : (
                      bookmarks.map((bm) => (
                        <div
                          key={bm.id}
                          className={`p-4 rounded-xl border ${activeStyle.border} flex flex-col gap-2`}
                        >
                          <div className="flex justify-between items-start">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-[9px] font-mono ${activeStyle.sub}`}
                            >
                  ...
                  <div className="p-4 space-y-6 text-neutral-400">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div
                          className={`p-4 rounded-2xl border ${activeStyle.border} bg-black/5 flex flex-col items-center text-center justify-center`}
                        >
                          <Clock size={20} className="mb-2 text-amber-600" />
                          <span
                            className={`text-[9px] uppercase font-bold ${activeStyle.sub}`}
                          >
                            Time Spent
                          </span>
                          <p className="text-sm font-mono font-bold">
                            {(readingStats.timeSpent / 60).toFixed(1)}h
                          </p>
                        </div>
                        <div
                          className={`p-4 rounded-2xl border ${activeStyle.border} bg-black/5 flex flex-col items-center text-center justify-center`}
                      ...
                      <div
                        className={`p-4 rounded-2xl border ${activeStyle.border} bg-black/5`}
                      >
                        <div className="flex justify-between items-center mb-3">
                          <span
                            className={`text-[10px] uppercase font-bold ${activeStyle.sub}`}
                          >
                            Scholar Progress
                          </span>
              ...
              <div className="p-6 border-t border-neutral-500/10 bg-black/5 dark:bg-white/5 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-[10px] ${activeStyle.sub}`}>
                      5 Days Reading Active
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            <div className="p-6 space-y-6 max-h-[400px] overflow-y-auto custom-scrollbar">
              {settingsTab === "themes" && (
                <div className="space-y-4">
                  <span
                    className={`text-[10px] font-bold ${activeStyle.sub} tracking-widest`}
                  >
                    ATMOSPHERE PRESETS
                  </span>
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className={`flex-1 h-12 rounded-xl border-2 ${activeStyle.border} hover:bg-black/5 flex items-center justify-center text-sm font-bold`}
                      >
                        A
                      </button>
                      <button
                        onClick={() => setFontSize(Math.min(48, fontSize + 1))}
                        className={`flex-1 h-12 rounded-xl border-2 ${activeStyle.border} hover:bg-black/5 flex items-center justify-center text-xl font-bold`}
                      >
                        A
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(Object.keys(fontStyles) as FontFamily[]).map((f) => (
                        <button
                          ...
                          style={{
                            fontFamily:
                              f === "dyslexic" ? "Atkinson Hyperlegible" : f,
                          }}
              ...
              {settingsTab === "layout" && (
                <div className="space-y-6">
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      {(Object.keys(marginStyles) as MarginSize[]).map((m) => (
                        ...
                        >
                          <div
                            className={`h-4 border-l-2 border-r-2 border-current ${m === "narrow" ? "w-6" : m === "normal" ? "w-4" : "w-2"}`}
                          />
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      {(Object.keys(spacingStyles) as LineSpacing[]).map(
                        (s) => (
                          <button
                            key={s}
            ...
            <div className="p-4 border-t border-neutral-500/10 bg-black/5 dark:bg-white/5 flex justify-between items-center">
              <span className={`text-[9px] font-mono ${activeStyle.sub}`}>
                DYNAMICS ENGINE ACTIVE
              </span>
              <div className="flex items-center gap-1">
        ...
        {showSearch && (
          ...
          >
            ...
            >
                ...
                {searchResults.length === 0 ? (
                  <div className="h-40 flex flex-col items-center justify-center opacity-30 italic text-sm text-neutral-400">
                    {searchQuery.length < 3
                      ? "Type 3+ characters to search locally..."
                      : "No lexical matches found nearby."}
                  </div>
                ) : (
                  <div className="space-y-2">
                    {searchResults.map((res, i) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          onUpdateProgress(
                            ...
                            res.chapterIndex,
                            Math.floor(
                              ((res.chapterIndex + 1) / chapters.length) * 100,
                            ),
                          );
                        }}
                      ...
                      >
                        <p
                          className={`text-[10px] font-bold ${activeStyle.sub} mb-1 group-hover:text-amber-600`}
                        >
                          {res.chapterTitle}
                        </p>
              ...
              <div
                className={`p-3 text-[9px] font-mono px-6 border-t ${activeStyle.border} opacity-40 bg-black/5`}
              >
        ...
        {activeSelectionText && (
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2.5">
                {[
                  ...
                  {
                    name: "emerald",
                    class: "bg-emerald-450 ring-emerald-500/30",
                  },
                  { name: "rose", class: "bg-rose-450 ring-rose-500/30" },
                  { name: "sky", class: "bg-sky-450 ring-sky-500/30" },
                  {
                    name: "underline",
                    class: "rainbow-underline border border-white/20",
                  },
                ].map((colorObj) => (
                  <button
                    key={colorObj.name}
                    onClick={() =>
                      setSelectedHighlightColor(colorObj.name as any)
                    }
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] uppercase tracking-wider font-bold text-neutral-400">
                Add Personal Annotation (Optional)
              </label>
              <textarea
