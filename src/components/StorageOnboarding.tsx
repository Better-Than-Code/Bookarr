...
export default function StorageOnboarding({
  onComplete,
}: StorageOnboardingProps) {
  ...
  const handleInitialize = async () => {
    ...
    if (isInIframe) {
      alert(
        "Native file access is restricted in the preview iframe. Please click 'Open in New Tab' at the top right to set up your local storage!",
      );
      return;
    }
    ...
    try {
      ...
      const downloadHandle = await handle.getDirectoryHandle("download", {
        create: true,
      });
