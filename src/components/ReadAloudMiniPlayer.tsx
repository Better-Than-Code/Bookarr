...
export default function ReadAloudMiniPlayer({ isReaderOpen = false, onOpenReader }: ReadAloudMiniPlayerProps) {
  const { 
    state, 
    pauseTts, 
    resumeTts, 
  ...
  const progressPercent = state.sentences.length > 0 
  ...
  const voicesByGroup = voices.reduce((acc, voice) => {
    const lang = voice.lang.split('-')[0];
    const langName = getLanguageName(lang);
    const category = isPremiumVoice(voice) ? `✨ ${langName} (Natural)` : `${langName} (Standard)`;
        ...
        <div className="max-w-7xl mx-auto px-4 py-2 flex flex-col">
          <div className="flex items-center justify-between h-14 relative">
              ...
              <div className="hidden sm:flex flex-col min-w-0">
                <span className="text-[10px] font-mono font-bold text-neutral-500 uppercase tracking-widest leading-none mb-1 flex items-center gap-2">
                  {state.isBuffering || state.neuralStatus === 'loading' ? (
                    <>
                      <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${state.engine === 'neural' ? 'bg-indigo-500' : 'bg-amber-500'}`} />
            ...
            <div className="flex items-center gap-1 w-1/4 justify-end">
              ...
              >
                <ChevronUp className={`transition-transform duration-300 ${isCollapsed ? 'rotate-0' : 'rotate-180'}`} size={18} />
              </button>
                  ...
                  {showSettings && (
                    ...
                    >
                      <div className="flex flex-col gap-5">
                                ...
                                {sortedVoiceGroupKeys.map(groupKey => (
                                  <optgroup label={groupKey} key={groupKey}>
                                    {voicesByGroup[groupKey].map((voice, idx) => (
                                      <option key={`${voice.voiceURI || voice.name}-${idx}`} value={voice.name}>
                                        {voice.name}
                                      </option>
                                    ))}
                                  </optgroup>
                            ...
                            <div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex flex-col gap-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                                        state.neuralStatus === 'ready' ? 'bg-emerald-500/20 text-emerald-400' :
                                        state.neuralStatus === 'loading' ? 'bg-indigo-500/20 text-indigo-400' :
                                        state.neuralStatus === 'error' ? 'bg-rose-500/20 text-rose-400' :
                                        'bg-neutral-500/20 text-neutral-400'
                                    }`}>
                                        {state.neuralStatus === 'ready' ? 'Verified Ready' : 
                                         state.neuralStatus === 'loading' ? 'Downloading...' :
                                         state.neuralStatus === 'error' ? 'Failed' : 'Idle'}
                                    </span>
                                </div>
                                ...
                                {state.neuralStatus === 'loading' && (
                                    <div className="space-y-1.5">
                                        <div className="text-[10px] text-neutral-400 space-y-1 mb-2">
                                            {state.neuralDownloadFileProgressList.map(file => (
                                                <div key={file.fileName} className="flex justify-between font-mono">
                                                    <span className="truncate mr-2">{file.fileName}</span>
                                                    <span>{file.status} {state.neuralDownloadIsIndeterminate ? `${(file.loaded / (1024 * 1024)).toFixed(1)} MB` : `${file.percentage}%`}</span>
                                                </div>
                                            ))}
                                        </div>
                                        <div className="flex items-center justify-between text-[10px]">
                                            <p className="text-neutral-500 font-medium text-[10px] leading-tight flex items-center gap-1.5">
                                                ...
                                                {state.neuralStatusMessage || 'Setting up...'}
                                            </p>
                                            <div className="flex items-center gap-2">
                                                {state.neuralDownloadSpeed && (
                                                    <span className="text-[9px] font-mono text-neutral-600 bg-neutral-900 px-1 py-0.5 rounded">{state.neuralDownloadSpeed}</span>
                                                )}
                                                <span className="text-indigo-400 font-bold min-w-[30px] text-right drop-shadow-[0_0_6px_rgba(99,102,241,0.5)]">
                                                    {state.neuralDownloadIsIndeterminate ? '' : `${Math.round(state.neuralDownloadProgress)}%`}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <div className="flex-1 h-2 bg-black/40 rounded-full overflow-hidden relative shadow-inner border border-white/5">
                                                <motion.div 
                                                    ...
                                                    initial={{ width: 0 }}
                                                    animate={state.neuralDownloadIsIndeterminate ? {
                                                      left: ["-30%", "100%"],
                                                      width: "30%"
                                                    } : { 
                                                    ...
                                                    transition={state.neuralDownloadIsIndeterminate ? {
                                                      duration: 1.5,
                                                      repeat: Infinity,
                                                      ease: "linear"
                                                    } : {
                                            ...
                                            <div className="flex items-center gap-1">
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); cancelNeuralEngine(); }}
                                                ...
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); resetNeuralEngine(); }}
                                ...
                                {state.neuralStatus === 'ready' && (
                                    <div className="space-y-1.5 mt-1">
                                        <p className="text-[10px] text-emerald-400/70 font-medium leading-tight flex items-center gap-1.5">
                                            <Sparkles size={10} />
                                            High-fidelity neural engine is active and ready.
                                        </p>
                                        <div className="py-1 px-2 mt-1 -mx-1 bg-emerald-500/10 rounded border border-emerald-500/20 text-[9px] font-mono text-emerald-300">
                                            {state.neuralHardware || 'Neural Engine Ready'}
                                        </div>
                                    </div>
                                )}
                                ...
                                {state.neuralStatus === 'error' && (
                                    <div className="space-y-1 mt-1">
                                        ...
                                        <p className="text-[9px] text-neutral-500 leading-tight">
                                            {state.neuralStatusMessage || 'Check internet connection.'}
                                        </p>
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); resetNeuralEngine(); }}
                        ...
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-mono font-bold text-neutral-500 uppercase tracking-widest">Speed: {state.speed.toFixed(2)}x</label>
                            ...
                            {[0.75, 1.0, 1.25, 1.5, 2.0].map((spd) => (
                              <button
                                ...
                                className={`py-2 text-[10px] font-bold rounded-lg border transition-all ${
                                  state.speed === spd 
                                    ? state.engine === 'neural' ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-amber-500 text-neutral-950 border-amber-500' 
              ...
              >
                <div className="pt-1 pb-4">
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5 flex items-start gap-3">
                    <Volume2 size={16} className={state.engine === 'neural' ? 'text-indigo-400' : 'text-amber-500'} />
