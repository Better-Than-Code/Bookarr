...
export default function MetadataSearchModal({
  ...
  const [providersStatus, setProvidersStatus] = useState<
    { name: string; status: "idle" | "loading" | "success" | "error" }[]
  >([
      ...
      >
        ...
        <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#181818]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-neutral-100 italic">
                Metadata Scraper
              </h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#0a0a0a] scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          ...
          {isAutoAdding && selectedCandidate && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h4 className="text-lg font-bold text-neutral-100 italic">
                High Confidence Match Found
              </h4>
              <p className="text-sm text-neutral-500 mt-1">
                Applying metadata from {selectedCandidate.source}...
              </p>
            ...
            >
              ...
              <div className="bg-[#141414] border border-white/10 rounded-2xl p-5 space-y-6">
                <div className="flex gap-6">
                  <img
                    src={selectedCandidate.coverUrl}
                    className="w-24 h-36 rounded-xl shadow-2xl border border-white/10"
                    alt=""
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <div className="flex items-center gap-1 text-[8px] font-black bg-neutral-800 text-neutral-400 px-1.5 py-0.5 rounded uppercase tracking-tighter border border-white/5">
                        {selectedCandidate.type === "audiobook" ? (
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {selectedCandidate.year && (
                        <span className="text-[10px] px-2 py-1 bg-black/40 text-neutral-500 rounded-lg font-bold border border-white/5">
                          {selectedCandidate.year}
                        </span>
                      )}
                      {selectedCandidate.publisher && (
                        <span className="text-[10px] px-2 py-1 bg-black/40 text-neutral-500 rounded-lg font-bold border border-white/5 truncate max-w-[120px]">
                          {selectedCandidate.publisher}
                        </span>
                      )}
                      {selectedCandidate.genres.map((g) => (
                        <span
                          key={g}
                          className="text-[10px] px-2 py-1 bg-black/40 text-neutral-500 rounded-lg font-bold border border-white/5"
                        >
                          {g}
                        </span>
                      ))}
                ...
                <div className="border-t border-white/5 pt-4">
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-neutral-600 mb-2">
                    Description Update
                  </h4>
                ...
                >
                  ...
                  <div className="relative">
                    <img
                      src={
                        res.coverUrl ||
                        "https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&q=80&w=100"
                      }
                    ...
                    <div className="absolute top-1 left-1 flex gap-1">
                      ...
                      <div className="bg-black/60 backdrop-blur-sm text-[8px] font-bold px-1 py-0.5 rounded border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center">
                        {res.type === "audiobook" ? (
                          <Music className="w-1.5 h-1.5 text-amber-500" />
                        ) : (
                          <BookOpen className="w-1.5 h-1.5 text-neutral-400" />
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0 py-1">
                    <div className="flex justify-between items-start">
                      <h4 className="font-bold text-neutral-100 truncate group-hover:text-amber-500 transition leading-tight">
                        {res.title}
                      </h4>
                      <div
                        className={`text-[10px] font-black px-1.5 py-0.5 rounded ml-2 ${
                          res.score > 0.8
                            ? "text-green-500"
                            : "text-neutral-500"
                        }`}
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {res.year && (
                        <span className="text-[9px] px-1.5 py-0.5 bg-neutral-800 text-neutral-500 rounded font-bold">
                          {res.year}
                        </span>
                      )}
                      {res.genres.slice(0, 2).map((g) => (
                        <span
                          key={g}
                          className="text-[9px] px-1.5 py-0.5 bg-neutral-800 text-neutral-500 rounded font-bold truncate max-w-[80px]"
                        >
                          {g}
                        </span>
                      ))}
                    ...
                    <p className="text-[10px] text-neutral-500 line-clamp-2 mt-2 leading-relaxed">
                      {res.description ||
                        "No summary available for this catalog entry."}
                    </p>
                  </div>
          ...
          {results.length === 0 &&
            ...
            !selectedCandidate && (
              <div className="text-center py-16 text-neutral-500">
                ...
                <p className="text-xs mt-1 max-w-[240px] mx-auto text-neutral-500">
