...
export default function BookDetailsModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(
        "Network error when saving updates: " + (e.message || "Unknown error"),
      );
    } finally {
      setIsSaving(false);
    }
      ...
      <div className="bg-[#121212] border border-[#222] rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] relative">
        {organizingStatus && (
          <div className="absolute inset-0 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center z-50 text-neutral-200 px-8 text-center animate-in fade-in duration-300">
            ...
            <h4 className="font-sans font-bold text-base mb-1 tracking-tight text-neutral-100">
              {organizingStatus}
            </h4>
            <p className="text-xs text-neutral-500 max-w-xs leading-relaxed">
              Please keep this browser window active. Writing directly to your
              device directories.
            </p>
          </div>
        )}
        ...
        <div className="p-4 border-b border-[#222] flex items-center justify-between bg-[#181818]">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-2 hover:bg-[#222] rounded-lg text-neutral-400 hover:text-white transition"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
              ...
              <div className="flex-1 space-y-5">
                {isEditing ? (
                  ...
                  <div className="space-y-2">
                    <h3 className="font-sans font-bold text-3xl text-neutral-100 leading-tight tracking-tight">
                      {book.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {book.genres.map((g) => (
                        <span
                          key={g}
                          className="text-[10px] font-bold bg-neutral-800 text-neutral-400 px-2.5 py-1 rounded-md border border-white/5 uppercase tracking-wider"
                        >
                          {g}
                        </span>
                      ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-[#181818] p-4 rounded-xl border border-white/5 flex flex-col justify-center">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div
                    className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${isAvailable ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
                  >
            ...
            <div className="pt-4 border-t border-[#222] flex flex-wrap gap-3">
              <button
                onClick={async () => {
                  ...
                  if (!hasPerms) {
                    alert(
                      "Permission to read file was denied. You may need to safely Re-Link the file from Configure or Sync folders again.",
                    );
                    return;
                  }
              ...
              <div className="flex gap-2 w-full md:w-auto">
                ...
                >
                  <RefreshCw
                    className={`w-4 h-4 ${isSyncingMeta ? "animate-spin" : ""}`}
                  />
                  Sync
                </button>
                ...
                {!isAvailable && (
                  <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-bold text-xs bg-neutral-800 text-amber-500 hover:bg-neutral-700 transition border border-amber-500/20 shadow-md shadow-amber-500/5 animate-pulse">
