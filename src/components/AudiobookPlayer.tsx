...
export default function AudiobookPlayer({
  ...
  const [activeMenu, setActiveMenu] = useState<
    "chapters" | "speed" | "sleep" | "bookmarks" | "stats" | null
  >(null);
  ...
  // Sleep Timer states
  const [sleepTimerType, setSleepTimerType] = useState<
    "minutes" | "chapter" | null
  >(null);
  ...
  const [bookmarks, setBookmarks] = useState<
    { id: string; time: number; label: string; createdAt: string }[]
  >([]);
  ...
  const [localChapterBlobUrl, setLocalChapterBlobUrl] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const defaultBms = [
        {
          id: `bm-def-1`,
          time: Math.floor(duration * 0.05),
          label: "Opening Remarks",
          createdAt: new Date().toLocaleDateString(),
        },
        {
          id: `bm-def-2`,
          time: Math.floor(duration * 0.35),
          label: "Key Narrative Twist",
          createdAt: new Date().toLocaleDateString(),
        },
      ];
      setBookmarks(defaultBms);
      localStorage.setItem(
        `bookrr-audiobook-bookmarks-${book.id}`,
        JSON.stringify(defaultBms),
      );
    }
  }, [book.id, duration]);
  ...
  const activeChapter: AudiobookChapter = (chapters[activeChapterIdx] ||
    chapters[0] || {
      id: "fallback",
  ...
  useEffect(() => {
    ...
    async function checkChapterOffline() {
      if (
        activeChapter &&
        activeChapter.id &&
        activeChapter.id !== "fallback"
      ) {
        try {
          const offlineChapter = await getOfflineChapterFile(
            book.id,
            activeChapter.id,
          );
          ...
          console.error(
            "Chapter LocalFile check failed in AudiobookPlayer:",
            e,
          );
        }
      }
  ...
  useEffect(() => {
    if (audioRef.current && audioRef.current.src !== currentAudioSrc) {
      ...
      if (wasPlaying) {
        audioRef.current
          .play()
          .catch((e) => console.error("Audio re-play failed:", e));
      }
    }
  ...
  const handleTimeUpdate = () => {
    if (audioRef.current) {
      ...
      const time = activeChapter.fileUrl
        ? activeChapter.start + audioTime
        : audioTime;
    ...
    if (isPlaying) {
      interval = setInterval(() => {
        ...
        setWaveHeights(
          Array.from(
            { length: 64 },
            () => Math.floor(Math.random() * multiplier) + minHeight,
          ),
        );
        ...
        if (sleepTimerType === "chapter") {
          if (
            activeChapterIdx !== -1 &&
            currentTime >= chapters[activeChapterIdx].end - 1
          ) {
            setIsPlaying(false);
            setSleepTimerType(null);
          }
  ...
  const handleAddBookmark = (e: React.FormEvent) => {
    e.preventDefault();
    const labelText =
      newBookmarkLabel.trim() || `Bookmark @ ${formatTime(currentTime)}`;
    const newBm = {
      id: `bm-${Date.now()}`,
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div className="fixed inset-0 z-[60] bg-[#0a0a0a] flex flex-col lg:hidden animate-in slide-in-from-bottom-full duration-300 pt-safe font-sans overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 px-5 z-10 bg-gradient-to-b from-[#0a0a0a] to-transparent">
            <button
              className="text-neutral-300 p-2 -ml-2 cursor-pointer hover:bg-neutral-900 rounded-full transition"
              onClick={(e) => {
                e.stopPropagation();
                setIsExpanded(false);
              }}
            ...
            <span className="text-[10px] font-bold text-neutral-400 tracking-[0.2em] font-mono">
              NOW LISTENING
            </span>
            <button
              className="text-neutral-300 p-2 -mr-2 cursor-pointer hover:bg-neutral-900 rounded-full transition"
              onClick={() =>
                setActiveMenu(activeMenu === "stats" ? null : "stats")
              }
            >
          ...
          <div className="flex-1 overflow-y-auto hide-scrollbar flex flex-col relative">
              ...
              <div className="absolute top-0 inset-x-4 bg-[#161616] border border-[#2d2d2d] rounded-2xl shadow-2xl p-5 z-20 text-xs font-mono text-neutral-400 space-y-3">
                <div className="flex justify-between items-center text-amber-500 font-bold uppercase border-b border-[#2d2d2d] pb-2">
                  <span className="flex items-center gap-2">
                    <Compass className="w-4 h-4" /> Stats
                  </span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-neutral-200">
                    {book.fileUrl?.split(".").pop()?.toUpperCase() || "Audio"}{" "}
                    format
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between border-t border-neutral-800/40 pt-2 text-[10px] text-neutral-500">
                  <span>Source:</span>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 min-h-[300px]">
              <div className="relative group shadow-2xl shadow-emerald-900/5">
                <img
                  src={book.coverUrl}
                  className="w-56 h-auto md:w-64 max-h-[45vh] aspect-square object-cover rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] ring-1 ring-white/10"
                />
              </div>
            </div>
            ...
            <div className="px-8 text-center pb-8 flex-none">
              <h2 className="text-xl md:text-2xl font-extrabold text-neutral-100 truncate tracking-tight">
                {book.title}
              </h2>
              <p className="text-sm md:text-base text-neutral-400 mt-1 truncate">
                {book.author}
              </p>
            ...
            <div className="flex items-center justify-center gap-8 pb-10 px-6 flex-none z-10">
              <button
                ...
                className="text-neutral-300 hover:text-white p-3 cursor-pointer"
              >
                <RotateCcw className="w-8 h-8" />
              </button>
              <button
                onClick={() => setIsPlaying(!isPlaying)}
                className="w-20 h-20 bg-amber-500 text-black rounded-full flex items-center justify-center cursor-pointer shadow-lg shadow-amber-500/20 active:scale-95 transition-transform"
              >
                {isPlaying ? (
            ...
            <div className="flex justify-around items-center px-4 md:px-12 pb-safe bg-gradient-to-t from-[#050505] to-transparent pt-4 mb-4 flex-none z-10 border-t border-neutral-900/50">
              <div className="relative flex flex-col items-center">
                <button
                  ...
                  className="flex flex-col items-center gap-1.5 text-neutral-400 p-2 cursor-pointer hover:text-white transition"
                >
                  ...
                  <div className="absolute bottom-16 bg-[#1a1a1a] border border-[#333] rounded-xl p-3 shadow-2xl flex flex-col gap-2 w-32 z-50 animate-in fade-in zoom-in-95">
                    {[1.0, 1.25, 1.5, 2.0].map((val) => (
                      <button
                        ...
                        className={`text-sm py-1.5 rounded-lg ${playbackRate === val ? "bg-amber-500 text-black font-bold" : "text-neutral-300 bg-neutral-800"}`}
                      >
                        {val.toFixed(2)}x
                      </button>
                    ))}
              ...
              <div className="relative flex flex-col items-center">
                <button
                  onClick={() =>
                    setActiveMenu(activeMenu === "sleep" ? null : "sleep")
                  }
                  className={`flex flex-col items-center gap-1.5 p-2 cursor-pointer transition ${sleepTimerType ? "text-amber-500" : "text-neutral-400 hover:text-white"}`}
                >
                  <Moon
                    className={`w-5 h-5 ${sleepTimerType ? "fill-amber-500/20" : ""}`}
                  />
                  <span className="text-[10px] font-semibold">
                    {sleepTimerType
                      ? formatCountdown(sleepTimerRemaining || 0).split(" ")[0]
                      : "Sleep"}
                  </span>
                </button>
                {activeMenu === "sleep" && (
                  <div className="absolute bottom-16 -ml-16 bg-[#1a1a1a] border border-[#333] rounded-xl p-3 shadow-2xl space-y-2 w-48 z-50 animate-in fade-in zoom-in-95">
                    <button
                      ...
                      className="w-full bg-neutral-800 text-white text-xs py-2 rounded-lg mix-blend-screen mb-1"
                    >
                      15 min
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="w-full bg-neutral-800 text-amber-500 font-bold text-xs py-2 rounded-lg mix-blend-screen mb-1"
                    >
                      End of Chapter
                    </button>
                    {sleepTimerType && (
                      <button
                        ...
                        className="w-full bg-rose-500/20 text-rose-500 font-bold text-xs py-2 rounded-lg mix-blend-screen border border-rose-500/30 font-mono"
                      >
                        Cancel
                      </button>
                    )}
              ...
              <div className="relative flex flex-col items-center">
                ...
                {activeMenu === "chapters" && (
                  <div className="absolute bottom-16 -ml-24 max-h-64 overflow-y-auto bg-[#1a1a1a] border border-[#333] rounded-xl p-2 w-64 shadow-2xl z-50 animate-in fade-in zoom-in-95 font-sans">
                    <div className="text-[10px] uppercase font-bold text-neutral-500 p-2 px-3 bg-neutral-900 mb-1 rounded flex justify-between">
                      <span>Chapters</span>
                      <X
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    {chapters.map((ch, idx) => (
                      <button
                        ...
                        className={`flex justify-between items-center w-full text-left p-3 rounded-xl mb-1 text-xs truncate transition ${currentTime >= ch.start && currentTime < ch.end ? "bg-amber-500 text-black font-bold" : "text-neutral-300 hover:bg-neutral-800"}`}
                      >
              ...
              <div className="relative flex flex-col items-center">
                <button
                  onClick={() =>
                    setActiveMenu(
                      activeMenu === "bookmarks" ? null : "bookmarks",
                    )
                  }
                  className={`flex flex-col items-center gap-1.5 p-2 cursor-pointer transition ${bookmarks.length > 0 ? "text-amber-500" : "text-neutral-400 hover:text-white"}`}
                >
                  ...
                  <div className="absolute bottom-16 -ml-32 max-h-64 overflow-y-auto bg-[#1a1a1a] border border-[#333] rounded-xl p-3 w-64 shadow-2xl z-50 animate-in fade-in zoom-in-95 font-sans flex flex-col gap-2">
                    ...
                    >
                      <input
                        ...
                        placeholder="Note..."
                        className="flex-1 bg-neutral-900 border border-neutral-700 p-1.5 text-xs rounded text-white"
                      />
                      <button
                        type="submit"
                        className="bg-amber-500 text-black px-2 rounded font-bold"
                      >
                    ...
                    {bookmarks.length === 0 ? (
                      ...
                      bookmarks.map((bm) => (
                        <div
                          ...
                          className="flex justify-between items-center bg-neutral-800 hover:bg-neutral-750 p-2 rounded-lg cursor-pointer"
                        >
      ...
      <div
        className={`fixed bottom-16 lg:bottom-0 left-0 lg:left-64 right-0 bg-[#121212]/98 backdrop-blur-xl border-t border-b lg:border-b-0 border-[#222] p-2 lg:p-4 flex-row items-center gap-2 lg:gap-6 z-[45] lg:z-50 shadow-2xl transition-all ${isExpanded ? "hidden lg:flex" : "flex"}`}
      >
        {/* 1. Audiobook Metadata Info */}
        <div
          className="flex items-center gap-2 lg:gap-4 flex-1 lg:flex-none lg:w-1/4 min-w-0 pr-2 lg:pr-0 border-r border-[#2d2d2d] lg:border-none cursor-pointer group"
        ...
        >
          ...
          <div className="text-left overflow-hidden min-w-0 flex-1">
            <h4 className="font-sans font-bold text-xs lg:text-sm text-neutral-100 truncate">
              {book.title}
            </h4>
            <p className="text-[10px] lg:text-[11px] text-neutral-400 truncate">
              {book.author}
            </p>
            <div className="hidden lg:block">
              <span className="inline-block mt-0.5 font-mono text-[9px] text-amber-500 font-semibold bg-amber-500/10 px-1.5 py-0.5 rounded truncate max-w-full">
                {activeChapterIdx !== -1 ? `Ch ${activeChapterIdx + 1}: ` : ""}
                {activeChapter.title}
              </span>
            </div>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`flex-1 rounded-full w-[1.5px] min-h-[3px] max-h-6 transition-all duration-300 ${
                      isFilled
                        ? vocalBoost
                          ? "bg-amber-300 shadow shadow-amber-400/40"
                          : "bg-amber-400 shadow shadow-amber-400/25"
                        : isPlaying
                          ? "bg-neutral-600"
                          : "bg-neutral-800"
                    }`}
        ...
        <div className="hidden lg:flex w-1/4 items-center justify-end gap-3.5 relative">
          ...
          >
            <Sparkles
              className={`w-4 h-4 ${vocalBoost ? "animate-pulse text-amber-400" : ""}`}
            />
                ...
                <div className="py-1">
                  {chapters.length === 0 ? (
                    <p className="p-3 text-xs text-neutral-500 italic">
                      No narration chapters configured
                    </p>
                  ) : (
                    chapters.map((ch, idx) => {
                      const isActive =
              ...
              className={`p-2 rounded-lg cursor-pointer transition border relative ${
                sleepTimerType
                  ...
                  : activeMenu === "sleep"
                    ...
                    : "text-neutral-400 border-transparent hover:bg-[#1a1a1a] hover:text-[#f3f4f6]"
              }`}
              title="Narration Sleep Alarm"
            >
              <Moon
                className={`w-4.5 h-4.5 ${sleepTimerType ? "animate-wink duration-1000" : ""}`}
              />
              {sleepTimerType && (
                ...
                {sleepTimerType ? (
                  <div className="bg-amber-500/5 border border-amber-500/20 p-2.5 rounded-lg space-y-1.5 text-xs text-center">
                    ...
                    <p className="text-amber-400 font-mono font-bold text-[15px] animate-pulse">
                      {sleepTimerType === "chapter"
                        ? "End of Chapter"
                        : formatCountdown(sleepTimerRemaining || 0)}
                    </p>
                    <button
                      onClick={stopSleepTimer}
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-neutral-200">
                      {activeChapterIdx !== -1 && chapters[activeChapterIdx]
                        ? formatTime(
                            chapters[activeChapterIdx].end - currentTime,
                          )
                        : "N/A"}
                    </span>
                  </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <input
              ...
              style={{
                backgroundImage: `linear-gradient(to right, #f59e0b 0%, #f59e0b ${volume * 100}%, #2d2d2d ${volume * 100}%, #2d2d2d 100%)`,
              }}
