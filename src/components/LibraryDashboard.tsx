...
export default function LibraryDashboard({
  ...
  const [onlineSelectedFormats, setOnlineSelectedFormats] = useState<{[key: string]: 'ebook' | 'audiobook'}>({});
  ...
  const [selectedInspectBook, setSelectedInspectBook] = useState<Book | null>(null);

  // Browser Storage / Sandbox Downloader state
  const [downloadingBook, setDownloadingBook] = useState<Book | null>(null);
  const [downloadProgress, setDownloadProgress] = useState<number>(0);
  const [downloadStatus, setDownloadStatus] = useState<'idle' | 'fetching' | 'saving' | 'completed' | 'error'>('idle');
  ...
  const handleCreateBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bookPayload: any = {
        ...
        size: selectedFile ? `${(selectedFile.size / (1024 * 1024)).toFixed(1)} MB` : (newType === 'audiobook' ? '180 MB' : '1.5 MB'),
        filePath,
        fileUrl
      };
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const formatted = docs.map((doc: any, i: number) => ({
          ...
          coverUrl: doc.cover_i ? `https://covers.openlibrary.org/b/id/${doc.cover_i}-L.jpg` : 'https://images.unsplash.com/photo-1543002588-bfa74002ed7e?auto=format&fit=crop&q=80&w=400',
          description: doc.first_sentence ? doc.first_sentence[0] : `A classical volume cataloged via online lookup.`,
          genres: doc.subject ? doc.subject.slice(0, 3) : ['General'],
          pages: doc.number_of_pages_median || doc.number_of_pages || 250,
          publishYear: doc.first_publish_year || doc.publish_year?.[0] || 'Unknown'
        }));
  ...
  const handleAddWantedBook = async (item: any) => {
    ...
    const bookPayload: any = {
      ...
      description: item.description || `Synopsis: Cataloged volume published in ${item.publishYear}.`,
      genres: item.genres,
    ...
    if (isAudio) {
      ...
      bookPayload.chapters = [
        { id: `ch-${Date.now()}-1`, title: 'Awaiting Download Content', start: 0, end: 600 }
      ];
    } else {
      bookPayload.pages = item.pages;
      bookPayload.chapters = [
        { id: `ch-${Date.now()}-1`, title: 'Overview', content: 'Point this book to a physical epub or trigger a search Tracker index below to fetch the book content.' }
      ];
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`"${item.title}" has been added to your Library as a "Wanted" entry. Hover over its list tile to search indexers or assign local files!`);
      } else {
  ...
  const handleInitiateDownload = async (book: Book) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`File fetch failed with status ${response.status}: ${response.statusText}`);
      }
      ...
      if (!reader) {
        ...
        finalBlob = new Blob(chunks, { 
          type: book.fileUrl.endsWith('.mp3') ? 'audio/mpeg' : 'application/epub+zip' 
        });
      }
  ...
  const triggerBlobDownload = async (blob: Blob, book: Book) => {
    try {
      ...
      if (watchHandle) {
        ...
        if (hasPerm) {
          try {
            ...
            console.log(`[STORAGE] Saved to internal storage: ${watchHandle.name}/${fileName}`);
            
            // Also update the offline record with this path
            await updateOfflineFilePath(book.id, `${watchHandle.name}/${fileName}`);
            await refreshOfflineBooks();
          } catch (storageErr) {
            console.error("Local storage save failed, falling back to download:", storageErr);
          }
        }
      }
      ...
      setDownloadStatus('error');
      setDownloadError("Sandbox constraints prevented automatic download triggering. Please use alternative methods.");
    }
  };
          ...
          <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-all duration-200">
            {!isAvailable ? (
              <div className="flex flex-col items-center gap-2 p-3 text-center">
                ...
                <span className="text-[10px] font-mono font-bold text-amber-400">Search Or Assign</span>
              </div>
            ) : isAudio ? (
              <button
                onClick={(e) => {
              ...
              <div className="flex items-center justify-between text-neutral-400">
                ...
                <button
                  onClick={(e) => {
                    ...
                    setCustomDevicePath(offlineBooksMap[book.id]?.filePath || `Downloads/${book.fileUrl?.split('/').pop() || (book.type === 'audiobook' ? `${book.title}.mp3` : `${book.title}.epub`)}`);
    ...
    <div className="space-y-6">
          ...
          <div className="flex bg-[#181818]/60 p-1 rounded-xl border border-neutral-900 w-fit">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-bold transition select-none cursor-pointer ${
                addMode === 'online' ? 'bg-amber-500 text-black shadow' : 'text-neutral-400 hover:text-neutral-200'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-bold transition select-none cursor-pointer ${
                addMode === 'manual' ? 'bg-amber-500 text-black shadow' : 'text-neutral-400 hover:text-neutral-200'
              }`}
            >
              <PlusCircle className="w-3.5 h-3.5" />
              Manual Custom Entry
          ...
          {addMode === 'online' ? (
            <div className="space-y-4">
              <p className="text-[11px] text-neutral-400 leading-snug font-sans">
                Search OpenLibrary's rich international catalog of millions of books and authors. Adding a title creates an elegant placeholder in your library with metadata and cover posters, ready to watch or search connected torrent indexers when you are!
              </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 flex flex-col justify-between min-w-0 space-y-2">
                          <div>
                            <h5 className="font-sans font-bold text-neutral-100 truncate" title={item.title}>
                              {item.title}
                            </h5>
                            ...
                            <p className="text-[9px] text-neutral-500 font-sans mt-0.5">
                              First Pub: {item.publishYear} • {item.pages || '?'} pgs
                            </p>
                          </div>
                              ...
                              <div className="flex items-center gap-2 bg-[#202025] px-1.5 py-0.5 rounded border border-neutral-850">
                                <label className="flex items-center gap-1 cursor-pointer">
                                  <input 
                                    ...
                                    onChange={() => setOnlineSelectedFormats(prev => ({ ...prev, [item.olId]: 'ebook' }))} 
                                    className="accent-amber-500 w-2.5 h-2.5"
                                  />
                                  <span className={format === 'ebook' ? 'text-amber-400 font-bold' : 'text-neutral-400'}>EBook</span>
                                </label>
                                <label className="flex items-center gap-1 cursor-pointer">
                                  <input 
                                    type="radio" 
                                    ...
                                    onChange={() => setOnlineSelectedFormats(prev => ({ ...prev, [item.olId]: 'audiobook' }))} 
                                    className="accent-amber-500 w-2.5 h-2.5"
                                  />
                                  <span className={format === 'audiobook' ? 'text-amber-400 font-bold' : 'text-neutral-400'}>Audio</span>
                                </label>
                              </div>
                            </div>
              ...
              {!isSearchingOnline && onlineSearchQuery && onlineResults.length === 0 && (
                <div className="py-12 text-center text-xs font-mono text-neutral-500 leading-normal">
                  No catalog records matching "{onlineSearchQuery}" found on OpenLibrary.<br />
                  Try searching a different title or shift to standard Manual Entry tab.
                </div>
              )}
            </div>
          ) : (
              ...
              <div className="md:col-span-2 space-y-1.5">
                ...
                <div className="flex items-center gap-3 bg-[#1a1a1a] border border-[#2d2d2d] p-3 rounded-xl border-dashed hover:border-amber-500/50 transition">
                   ...
                   <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-neutral-400 mb-1">Upload direct from browser to staging</p>
                      <input 
                        type="file" 
      ...
      {selectedBookDetails && (
        <BookDetailsModal
          ...
          onRead={onReadEbook}
          isAvailable={isBookAvailable(selectedBookDetails.id, selectedBookDetails.isDownloaded)}
        ...
        >
          ...
          >
            ...
            <div className={`absolute top-0 left-0 right-0 h-1.5 ${
              downloadStatus === 'error' ? 'bg-rose-500' :
              downloadStatus === 'completed' ? 'bg-emerald-500' : 
              'bg-amber-500'
            }`} />
            ...
            <div className="flex items-start gap-3.5 mb-5 mt-2">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                downloadStatus === 'error' ? 'bg-rose-500/10 text-rose-400' :
                downloadStatus === 'completed' ? 'bg-emerald-500/10 text-emerald-400' :
            ...
            <div className="bg-[#16161f] border border-neutral-900 rounded-xl p-3 mb-5 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-neutral-200 truncate leading-tight">{downloadingBook.title}</p>
                <p className="text-[10px] text-neutral-400 mt-0.5 truncate">{downloadingBook.author}</p>
            ...
            <div className="space-y-4 mb-6">
              {downloadStatus === 'fetching' && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="font-mono font-bold text-amber-500">{downloadProgress}%</span>
                  </div>
                  {/* Progress bar */}
                  ...
                  <p className="text-[10px] text-neutral-500 leading-snug pt-1">
                    Fetching locally allows us to place a copy in your browser database AND stream it offline.
                  </p>
                </div>
              )}
              ...
              {downloadStatus === 'saving' && (
                <div className="text-center py-4 space-y-3">
                  ...
                  <p className="text-xs text-neutral-300 font-medium">Assembling file data, persisting in browser store & saving...</p>
                </div>
              )}
              ...
              {downloadStatus === 'completed' && (
                <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-3 text-left space-y-1">
                  ...
                  <p className="text-[10px] text-neutral-300 leading-relaxed">
                    The file was saved to your device. Check your browser Downloads folder. It is also cached offline in browser storage!
                  </p>
                </div>
              )}
              ...
              {downloadStatus === 'error' && (
                <div className="bg-rose-500/5 border border-rose-500/20 rounded-xl p-3 text-left space-y-1">
                  ...
                  <p className="text-[10px] text-neutral-300 leading-normal">
                    {downloadError || "Your browser's iframe security sandbox blocked the automated file save command."}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2 px-1 pt-3 border-t border-neutral-900">
              ...
              <p className="text-[9px] text-neutral-500 text-center leading-normal mt-2.5">
                💡 Opening either option in a new tab operates completely free of the sandboxed workspace!
              </p>
            </div>
            ...
            <div className="mt-5 flex justify-end">
              ...
              >
                {downloadStatus === 'fetching' || downloadStatus === 'saving' ? 'Cancel / Hide' : 'Close'}
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            <div className="bg-[#12121a] border border-neutral-900 rounded-xl p-3 mb-4 flex items-center gap-3">
              ...
              />
              <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-neutral-200 truncate leading-tight">{configuringBookPath.title}</p>
                <p className="text-[10px] text-neutral-400 mt-0.5 truncate">{configuringBookPath.author}</p>
                <span className="inline-block text-[9px] bg-[#1a1a24] text-amber-400 font-mono rounded px-1.5 py-0.5 mt-1.5">
                  {configuringBookPath.type === 'audiobook' ? 'AUDIOBOOK' : 'EBOOK'}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-5">
              <div className="space-y-1">
                  ...
                  <button
                    onClick={async () => {
                      if (!customDevicePath.trim()) {
                        alert("Please provide a valid file path representation.");
                        return;
                      }
                      
                      // Check if we already have it in IndexedDB, if not we seed with empty blob to preserve the path string!
                      const offlineRecord = await getOfflineFile(configuringBookPath.id);
                      if (offlineRecord) {
                        await updateOfflineFilePath(configuringBookPath.id, customDevicePath.trim());
                      } else {
                        // Create a dummy placeholder blob to mark it as local
                        const placeholderBlob = new Blob(["dummy content"], { type: configuringBookPath.type === 'audiobook' ? 'audio/mpeg' : 'application/epub+zip' });
                        await saveOfflineFile(
                          configuringBookPath.id, 
                          configuringBookPath.fileUrl?.split('/').pop() || `${configuringBookPath.title}.epub`, 
                          placeholderBlob, 
                ...
                <p className="text-[9px] text-neutral-500 font-sans leading-snug pt-0.5">
                  Keep track of where you saved the download. Inputting the exact system path helps you remember!
                </p>
              </div>
              ...
              <div className="pt-3 border-t border-neutral-900 space-y-2">
                ...
                <input 
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      await saveOfflineFile(configuringBookPath.id, safeName, blob, guessedPath);
                      await refreshOfflineBooks();
                      
                      setCustomDevicePath(guessedPath);
                      setLinkSuccessMessage(`Pointed book map to localized version: ${safeName}`);
                    } catch (err: any) {
                      alert(`Failed to load selected physical file: ${err.message}`);
                    }
                  }}
                />
                ...
                <button
                  onClick={() => {
                    document.getElementById('native-device-file-picker')?.click();
                  }}
                ...
                <p className="text-[9px] text-neutral-500 leading-snug">
                  Select the downloaded file on your computer/phone. Reading it locally loads it directly in your browser's persistent offline database!
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-neutral-950">
              <button
                onClick={async () => {
                  if (confirm("Are you sure you want to unlink and remove browser memory storage? Your physical file in the Downloads folder won't be modified.")) {
      ...
      {selectedInspectBook && (() => {
        ...
        const isAvailable = isBookAvailable(selectedInspectBook.id, selectedInspectBook.isDownloaded);
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div className={`absolute top-0 left-0 right-0 h-1.5 ${isAvailable ? 'bg-emerald-500' : 'bg-amber-500'}`} />
              ...
              <div className="flex flex-col sm:flex-row gap-5 items-start">
                ...
                <div className="flex-1 min-w-0 space-y-2 mt-1 w-full">
                  <h3 className="font-sans font-extrabold text-[#eeeeee] text-base sm:text-lg leading-snug truncate" title={selectedInspectBook.title}>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 pt-1">
                    {selectedInspectBook.genres.map(g => (
                      <span key={g} className="text-[9px] bg-neutral-900 border border-neutral-800 rounded-md px-2 py-0.5 text-neutral-400 font-sans font-medium">
                        {g}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="text-[10px] text-neutral-400 font-mono space-y-1 pt-1.5 border-t border-neutral-900">
                    <p>Format: <span className="text-neutral-200 capitalize">{selectedInspectBook.type}</span></p>
                    <p>Size: <span className="text-neutral-200">{selectedInspectBook.size || 'Auto-Sized'}</span></p>
                    <p>
                      Status: {isAvailable ? (
                        <span className="text-emerald-400 font-bold bg-emerald-400/10 px-1.5 py-0.5 rounded">
                          Available / Downloaded {hasOfflineCopy ? '(Offline cached)' : '(Native server)'}
                        </span>
                      ) : (
              ...
              {/* Description Section */}
              <div className="space-y-1.5">
                <h4 className="text-[10px] uppercase font-mono tracking-wider text-neutral-500 font-bold">Synopsis / Description:</h4>
                <div className="bg-[#121217] rounded-xl p-3 border border-neutral-900 text-[11px] leading-relaxed text-neutral-300 font-sans max-h-24 overflow-y-auto">
                  {selectedInspectBook.description || 'No digital synopsis listed for this cataloged volume reference.'}
                </div>
              </div>
                ...
                {!isAvailable ? (
                  <div className="space-y-3">
                    <p className="text-[10px] text-neutral-400 leading-normal font-sans">
                      ⚠️ This volume has been added as a <strong>Wanted Bookmark</strong> details. You can instantly trigger tracker scans on indexers to request this title file or pick locally:
                    </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      ...
                      <button
                        onClick={() => {
                          const query = `${selectedInspectBook.title} ${selectedInspectBook.author}`.replace(/[^a-zA-Z0-9\s]/g, ' ');
                      ...
                      <input 
                        ...
                        onChange={async (e) => {
                          ...
                          const file = files[0];
                          try {
                            setLinkSuccessMessage("Binding offline file representation...");
                            const blob = new Blob([file], { type: file.type });
                            const defaultDevicePath = `Downloads/${file.name}`;
                            await saveOfflineFile(selectedInspectBook.id, file.name, blob, defaultDevicePath);
                            await refreshOfflineBooks();
                            
                            setLinkSuccessMessage(`Offline copy linked! ${file.name} is now ready to read.`);
                            ...
                            alert("Failed to map file locally: " + err.message);
                          }
                        }}
                      />
                      ...
                      <button
                        onClick={() => {
                          document.getElementById('wanted-file-picker')?.click();
                        }}
                  ...
                  <div className="flex flex-col gap-2">
                    <p className="text-[10px] text-neutral-400 leading-normal font-sans">
                      ✅ File is available in device storage or local web cache database. Ready for playback:
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <button
                        onClick={() => {
                          setSelectedInspectBook(null);
                          isAudio ? onPlayAudiobook(selectedInspectBook) : onReadEbook(selectedInspectBook);
                        }}
                      ...
                      >
                        {isAudio ? <Play size={13} className="fill-current" /> : <BookOpen size={13} />}
                        {isAudio ? 'Listen Audiobook' : 'Read E-book'}
                      </button>

                      {(selectedInspectBook.fileUrl || hasOfflineCopy) && !hasOfflineCopy && (
                        <button
                          onClick={() => {
                            setSelectedInspectBook(null);
              ...
              <div className="flex justify-between gap-2 pt-3 border-t border-neutral-950 items-center">
                <button
                  onClick={() => {
                    if (confirm(`Remove "${selectedInspectBook.title}" completely from index bookmarks? Any associated browser storage will be unlinked.`)) {
