...
interface ReadAloudContextValue {
  ...
  registerCallback: (onNextChapter?: () => void, onSentenceChange?: (index: number) => void) => void;
...
export const ReadAloudProvider = ({ children }: { children: ReactNode }) => {
  const [state, setState] = useState<ReadAloudState>(() => {
    return {
      ...
      selectedVoiceName: '',
      chapterIndex: 0,
      engine: (localStorage.getItem('bookrr-pref-engine') as 'native' | 'neural') || 'native',
      ...
      selectedModelId: localStorage.getItem('bookrr_tts_model_id') || 'kokoro/Kokoro-82M-v1.0-ONNX',
    };
  });

  const utteranceRef = useRef<SpeechSynthesisUtterance | null>(null);
  
  const getAudioKey = (bookId: string, chapIdx: number, sentenceIdx: number, speed: number, voice: string) => 
  ...
  const preloadedAudioRef = useRef<Record<string, {audio: Float32Array | Blob | ArrayBuffer, samplingRate: number, isPiperTransfer?: boolean}>>({});
  const requestedAudioRef = useRef<Record<string, boolean>>({});
  const onAudioReadyCallbackRef = useRef<Record<string, (audio: Float32Array | Blob | ArrayBuffer, sr: number, isPiper?: boolean) => void>>({});
  ...
  const onSentenceChangeRef = useRef<((index: number) => void) | undefined>(undefined);

  const registerCallback = (onNextChapter?: () => void, onSentenceChange?: (index: number) => void) => {
  ...
  const initWorker = (force = false) => {
    console.log(`[TTS Context] initWorker called. force=${force}, currentWorker=${!!workerRef.current}`);
    ...
    if (!workerRef.current && typeof window !== 'undefined') {
        const customModelId = localStorage.getItem('bookrr_tts_model_id') || 'kokoro/Kokoro-82M-v1.0-ONNX';
        ...
        worker.onmessage = (e) => {
            ...
            if (type === 'audio') {
                if (reqId) {
                    console.log(`[TTS Analytics] Engine Audio TTFB: reqId=${reqId}, Duration: ${durationMs}ms`);
                    console.log(`[TTS Context] Worker audio recv reqId=${reqId}, isPiperTransfer=${isPiperTransfer}`);
                    preloadedAudioRef.current[reqId] = { audio, samplingRate, isPiperTransfer };
                    if (onAudioReadyCallbackRef.current[reqId]) {
                        console.log(`[TTS Context] Preloaded audio ready for ${reqId}, playing now.`);
                        onAudioReadyCallbackRef.current[reqId](audio, samplingRate, isPiperTransfer);
                ...
                console.error(`[TTS Context] Worker Error for reqId: ${reqId}: ${message}`);
                ...
                setState(prev => ({ ...prev, neuralStatus: 'ready', neuralDownloadProgress: 100, neuralDownloadIsIndeterminate: false, isBuffering: false, neuralDownloadSpeed: '', neuralDownloadFileProgressList: [] }));
                ...
                setState(prev => ({ 
                  ...prev, 
                  isBuffering: (message === 'Synthesizing...' || message === 'Synthesizing (Piper WASM)...') && !reqId, // only show buffering globally if not prefetching
                  neuralStatus: (message === 'Synthesizing...' || message === 'Synthesizing (Piper WASM)...') ? prev.neuralStatus : 'loading',
                ...
                setState(prev => ({ ...prev, isBuffering: false, neuralStatus: 'error', neuralStatusMessage: message, neuralDownloadSpeed: '' }));
            }
        };
        worker.onerror = (err) => {
            console.error("[TTS Context] Worker DOM Error:", err);
            setState(prev => ({ ...prev, isBuffering: false, neuralStatus: 'error' }));
        };
        workerRef.current = worker;
        
        const customBackend = localStorage.getItem('bookrr_tts_backend') || 'auto';
        worker.postMessage({ type: 'init', force: force, modelId: customModelId, backend: customBackend });
    } else if (workerRef.current) {
        const customModelId = localStorage.getItem('bookrr_tts_model_id') || 'kokoro/Kokoro-82M-v1.0-ONNX';
        const customBackend = localStorage.getItem('bookrr_tts_backend') || 'auto';
        workerRef.current.postMessage({ type: 'init', force: force, modelId: customModelId, backend: customBackend });
    }
  ...
  const loadNeuralModelManually = (file: File) => {
      console.log("[TTS Context] loadNeuralModelManually triggered for file:", file.name);
      if (!workerRef.current) {
          initWorker();
      }
      if (workerRef.current) {
          setState(prev => ({ ...prev, neuralStatus: 'loading', neuralStatusMessage: `Uploading ${file.name}...` }));
  ...
  const playRawAudio = (audioData: Float32Array | Blob | ArrayBuffer, sampleRate: number, isPiperTransfer: boolean = false, onEnded: () => void) => {
    ...
    let audioUrl: string = '';
    try {
        const isBlob = audioData instanceof Blob || (audioData && (audioData as any).size !== undefined && (audioData as any).type !== undefined);
        console.log("[TTS Context] playRawAudio started. isBlob:", isBlob, "typeof audioData:", typeof audioData);
        
        if (isPiperTransfer) {
            let properBlob = new Blob([audioData as ArrayBuffer], { type: 'audio/wav' });
            audioUrl = URL.createObjectURL(properBlob);
            console.log("[TTS Context] Created audioUrl from Piper Transfer:", audioUrl);
        } else if (isBlob) {
            let properBlob = new Blob([audioData as Blob], { type: 'audio/wav' });
            audioUrl = URL.createObjectURL(properBlob);
            console.log("[TTS Context] Created audioUrl from Blob:", audioUrl);
        } else {
            console.log("[TTS Context] Found Float32Array, length:", (audioData as Float32Array).length);
            const wavBlob = encodeWAV(audioData as Float32Array, sampleRate);
            audioUrl = URL.createObjectURL(wavBlob);
            console.log("[TTS Context] Created audioUrl from WAV encode:", audioUrl);
        }
    } catch (e) {
    ...
    const handleEnded = () => {
        ...
        if (isSpeakingRef.current && !isPausedRef.current) {
            ...
            console.log("[TTS Context] Playback ended/aborted but player is paused or stopped. Skipping automatic advancement.");
        }
    };
    ...
    audio.onerror = (e) => {
        ...
        if (isSpeakingRef.current && !isPausedRef.current) {
            ...
            console.log("[TTS Context] Suppressed error advancement because player is not in active speaking state.");
        }
    };
    audio.load();
    ...
    if (playPromise !== undefined) {
      playPromise.catch(e => {
          console.warn("[TTS Context] Audio playback playPromise failed/aborted:", e.name, e.message || e);
          if (e.name === 'AbortError') {
              console.log("[TTS Context] Playback aborted intentionally. Ignoring auto-advancement.");
              return;
          }
          if (e.name === 'NotAllowedError') {
              console.log("[TTS Context] Playback not allowed. Setting player to paused state to allow user interaction.");
          ...
          if (isSpeakingRef.current && !isPausedRef.current) {
              ...
              console.log("[TTS Context] Suppressed abort/play failure advancement because player is not in active speaking state.");
          }
      });
    }
  };
  ...
  const startSpeakingFromIndex = (
    ...
    if (engineVal === 'neural') {
        ...
        if (worker) {
            ...
            const reqKey = getAudioKey(bookIdVal || '', chapIdxVal, indexToSpeak, speedVal, safeNeuralVoice);
            ...
            const playCallback = (audio: Float32Array | ArrayBuffer | Blob, samplingRate: number, isPiperTransfer: boolean = false) => {
                ...
                playRawAudio(audio, samplingRate, isPiperTransfer, () => {
                    startSpeakingFromIndex(indexToSpeak + 1, sentencesList, speedVal, voiceNameVal, engineVal, bookIdVal, bookTitleVal, chapIdxVal);
                });
            ...
            for (let i = -1; i <= 5; i++) {
                if (i !== 0 && indexToSpeak + i >= 0 && indexToSpeak + i < sentencesList.length) {
                    const key = getAudioKey(bookIdVal || '', chapIdxVal, indexToSpeak + i, speedVal, safeNeuralVoice);
            ...
            worker.postMessage({ type: 'set_active_reqs', activeReqIds: workerActiveReqIds });

            if (preloadedAudioRef.current[reqKey]) {
                console.log(`[TTS Context] Using preloaded audio for ${reqKey}`);
                playCallback(preloadedAudioRef.current[reqKey].audio, preloadedAudioRef.current[reqKey].samplingRate, preloadedAudioRef.current[reqKey].isPiperTransfer);
            } else {
                ...
                if (!requestedAudioRef.current[reqKey]) {
                    ...
                    worker.postMessage({ type: 'speak', text: textToSpeak, speed: speedVal, modelId: customModelId, backend: localStorage.getItem('bookrr_tts_backend') || 'auto', voice: safeNeuralVoice, reqId: reqKey });
                }
            }
            ...
            for (let i = 1; i <= 4; i++) {
                ...
                if (prefetchIdx < sentencesList.length) {
                    ...
                    if (nextText && nextText.trim()) {
                        const prefetchKey = getAudioKey(bookIdVal || '', chapIdxVal, prefetchIdx, speedVal, safeNeuralVoice);
                        if (!preloadedAudioRef.current[prefetchKey] && !requestedAudioRef.current[prefetchKey]) {
                            console.log(`[TTS Context] Prefetching ${prefetchKey}`);
                            requestedAudioRef.current[prefetchKey] = true;
                            worker.postMessage({ type: 'speak', text: nextText, speed: speedVal, modelId: customModelId, backend: localStorage.getItem('bookrr_tts_backend') || 'auto', voice: safeNeuralVoice, reqId: prefetchKey });
                        }
                    }
                }
            }
  ...
  const startSpeaking = (
    ...
    startSpeakingFromIndex(startIndex, sentences, speed, voiceName, engine, bookId, bookTitle, chapterIndex);
  };
  ...
  const setSpeed = (speed: number) => {
    ...
    if (state.isSpeaking && state.bookId) {
      startSpeakingFromIndex(state.currentIndex, state.sentences, speed, state.selectedVoiceName, state.engine);
    }
  };
  ...
  const setVoiceName = (voiceName: string) => {
    ...
    if (state.isSpeaking && state.bookId) {
      startSpeakingFromIndex(state.currentIndex, state.sentences, state.speed, voiceName, state.engine);
    }
  };
  ...
  const setEngine = (engine: 'native' | 'neural') => {
    ...
    if (state.isSpeaking && state.bookId) {
      startSpeakingFromIndex(state.currentIndex, state.sentences, state.speed, state.selectedVoiceName, engine);
    }
  };
  ...
  const nextSentence = () => {
    ...
    if (nextIdx < state.sentences.length) {
      startSpeakingFromIndex(nextIdx, state.sentences, state.speed, state.selectedVoiceName, state.engine);
    }
  };
  ...
  const prevSentence = () => {
    ...
    if (prevIdx >= 0) {
      startSpeakingFromIndex(prevIdx, state.sentences, state.speed, state.selectedVoiceName, state.engine);
    }
  };

  const setCurrentIndex = (index: number) => {
    if (index >= 0 && index < state.sentences.length) {
      startSpeakingFromIndex(index, state.sentences, state.speed, state.selectedVoiceName, state.engine);
    }
  };
